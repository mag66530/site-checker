...
# id проекта для имени секрета (autoclick_session_<pid>). Для этого архива smu.
PID = os.environ.get('SITE_CHECKER_PID', 'smu').strip() or 'smu'
# Домены, чьи cookies нужны (Вебмастер + GSC) - остальное в облако не тащим.
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # ── Сохраняем вход ──
        # (1) session.json — для ЛОКАЛЬНОЙ проверки (2-PROVERKA грузит его).
        try:
            await ctx.storage_state(path=str(SESSION))
            _log(f'✅ session.json сохранён — локальную проверку (2-PROVERKA) '
                 'можно запускать без входа.')
        except Exception as e:
            _log(f'⚠ не удалось сохранить session.json: {e}')
        # (2) base64 сессии — для ОБЛАКА (Streamlit Secrets), только cookies
        # Яндекса/Google. Вставить в Secrets ключом autoclick_session_<pid> —
        # тогда проверка работает на сайте для ВСЕХ, с любого компа, без входа.
        try:
            state = await ctx.storage_state()
            cookies = [c for c in state.get('cookies', [])
                       if any(k in (c.get('domain') or '').lower()
                              for k in _KEEP)]
            ya = sum(1 for c in cookies
                     if 'yandex' in (c.get('domain') or '').lower()
            ...
            b64_file = ROOT / f'autoclick_session_{PID}.b64'
            b64_file.write_text(b64, encoding='ascii')
            _log(f'✅ Для облака: {b64_file.name} (cookies Яндекс {ya}, '
                 f'Google {len(cookies) - ya}).')
            _log(f'   На Streamlit: Settings → Secrets → добавь '
                 f'autoclick_session_{PID} = "<содержимое {b64_file.name}>".')
        except Exception as e:
            _log(f'⚠ не удалось сделать base64 для облака: {e}')
