...
def build_report(
    *,
    ...
    if metrika_reports:
        ...
        # Пояснение сшивки — простыми словами, без жаргона.
        # Сортировка задаётся отдельно (подтверждённые сверху, дальше по
        # числу просмотров) — здесь объясняем что значат цветные плашки.
        ws4.merge_cells('A3:H3')
        c = ws4['A3']
        c.value = (
            'Что означают цвета в колонке «Статус»:   '
            '🔴 Точно сломан — страница не открылась и в Метрике, и в нашей проверке сайта. '
            'Это двойное подтверждение, чинить в первую очередь.   '
            '⚠ Только в Метрике — Метрика зафиксировала 404, но мы в этом прогоне '
            'эту страницу не проверяли (или её не было в выборке).   '
            'Сортировка: сначала «Точно сломан», потом по числу просмотров.'
        )
        c.font = _font(size=9, italic=True, color=C.text_muted)
        c.alignment = _align(wrap=True, vertical='top')
        ws4.row_dimensions[3].height = 56
