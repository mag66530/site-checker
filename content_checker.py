...
BLOCK_DESCRIPTIONS = {
    'photos':        'Фото товаров. Проверяем, у всех ли товаров есть СВОЁ фото. «✓» – фото у всех на месте. «БАГ (5)» – у 5 товаров вместо фото стоит стандартная картинка-заглушка завода (no-image), которую сайт ставит автоматически, когда у товара нет своего фото. Визуально картинка есть, но это «нет изображения», а не реальное фото товара – это и ловим. «–» – на странице нет карточек товаров (например, на корне каталога только разделы), проверять нечего.',
