...
# пробелы (в т.ч. неразрывные) в числе, плюс сокращения «р.»/«с.».
_PRICE_RE = re.compile(r'\d[\d\s\u00a0]{0,12}(?:[.,]\d{1,2})?\s*(?:[₽₸₼֏]|руб|тенге|тг\.?|сум|сўм|сом|манат|драм|uzs|kzt|kgs|byn|azn|amd|rub|р\.|с\.)', re.IGNORECASE)
...
BLOCK_DESCRIPTIONS = {
    'photos':        'Фото товаров. Проверяем, у всех ли товаров есть фото. «✓» – фото у всех на месте. «БАГ (5)» – у 5 товаров вместо фото пустая картинка-заглушка (это ошибка, число = сколько товаров без фото). «–» – на странице нет товаров (например, на главной каталога только разделы), проверять нечего.',
...
def check_content(html: str, type_code: str, css_hidden: tuple = (),
    ...
    for blk in _profile:
        ...
        if blk.key == 'rec_price' and not _rec_has_cards(ctx):
            required = False
        # «Фото товаров» на КОРНЕ каталога проверяем только если там есть карточки
        # товаров («Популярные позиции»). Без них заглушка picture.missing – это
        # плитки РАЗДЕЛОВ (у разделов нет картинки), а не «фото товара»: пункт
        # необязателен, в отчёте «–», а не ложный баг. На листинге/товаре фото
        # проверяем как обычно (там карточка/товар точно есть).
        if (blk.key == 'photos' and type_code == 'catalog'
                and not _d_product_cards(ctx)[0]):
            required = False
