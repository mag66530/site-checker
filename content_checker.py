...
_PHONE_RE = re.compile(
    ...
    # Беларусь: +375 (44) 588-81-48, а также БЕЗ «+» - mepen.by пишет
    # «375 (29) 643-66-60» и tel:375296436660 (как у 998/996/994 «+» опционален).
...
class _Ctx:
    ...
    # Город ЭТОЙ страницы (из карты присутствия). Нужен проверке «выбор города
...
_КЛИКАБЕЛЬНЫЙ_RE = re.compile(r'<(a|button)\b[^>]*>(.*?)</\1>',
                              re.IGNORECASE | re.DOTALL)


def _норм_подпись(s: str) -> str:
    """Подпись элемента к сравнимому виду: без тегов, регистра и «ё»."""
    без_тегов = re.sub(r'<[^>]+>', ' ', s or '')
    return re.sub(r'\s+', ' ', без_тегов).strip().lower().replace('ё', 'е')


def _город_кликабелен(header_html: str, city: str) -> bool:
    """Есть ли в шапке кликабельный элемент, подписанный ГОРОДОМ этой страницы.

    Так свёрстан переключатель у сайтов, где слова «город» в подписи нет:
    SHOPMET и МПИ рисуют <button>Самара</button> / <button>Москва</button>.
    Сверяем с городом ИМЕННО этой страницы (прогон знает его из карты
    присутствия), поэтому «Самара» в адресе подвала или в тексте за
    переключатель не сойдёт.

    Города вида «Астана/Нур-Султан» на сайте подписаны одной половиной -
    принимаем любую.

    Сравниваем НАБОРЫ слов, а не строки: у SHOPMET внутри кнопки город стоит
    дважды («Самара Самара» - вторая копия для всплывающей подсказки), у
    других бывает иконка или значок-стрелка. Зато «Москва, ул. Ленина, д. 24»
    из кнопки адреса не пройдёт - там слов больше, чем в названии города.
    ЧИСТАЯ функция (юнит-тест без сети)."""
    искомые = []
    for часть in re.split(r'[/\\]', city or ''):
        слова = set(_норм_подпись(часть).split())
        if слова:
            искомые.append(слова)
    if not искомые:
        return False
    for m in _КЛИКАБЕЛЬНЫЙ_RE.finditer(header_html or ''):
        # Подпись у самого элемента (title="Самара") - самый явный признак.
        подписи = re.findall(r'title=["\']([^"\']{1,40})["\']', m.group(0), re.I)
        if any(set(_норм_подпись(т).split()) in искомые for т in подписи):
            return True
        if set(_норм_подпись(m.group(2)).split()) in искомые:
            return True
    return False
...
def _d_hdr_city(c: _Ctx):
    ...
    present = (
        ...
        # Подпись = сам город (SHOPMET, МПИ): кнопка «Самара» без слова «город».
        or _город_кликабелен(c.header_html, c.city)
    )
    return present, None
...
def _d_ftr_writeus(c: _Ctx):
    ...
    present = (
        ...
        # Короткое «Написать» (SHOPMET) - только когда в подвале есть почта:
        # тогда это та самая кнопка связи, а не «написать отзыв» из чужого
        # блока. Без почты короткое слово не засчитываем.
        or ('написать' in t and bool(_EMAIL_RE.search(c.footer_html)))
...
# Отзыв с микроразметкой Schema.org Review (itemtype=.../Review) - самый
# надёжный признак реального блока отзывов на странице (а не пункта меню).
_REVIEW_MARK = 'itemtype="https://schema.org/review"'


def _d_reviews(c: _Ctx):
    # Проверка ТОЛЬКО на наличие отзывов на странице (без разбора источников/
    # ссылок): либо микроразметка Schema.org Review, либо заголовок блока
    # отзывов в тексте. Одиночное слово «Отзывы» в меню/футере не считаем -
    # это ссылка на раздел, а не сам блок с отзывами.
    present = (
        _REVIEW_MARK in c.html_lower
        or 'отзывы клиентов' in c.text_lower
        or 'отзывы покупателей' in c.text_lower
        or 'отзывы наших' in c.text_lower
        or 'отзывы о ' in c.text_lower
    )
    return present, None
...
# Маркеры нижних блоков-каруселей (рекомендации / «ранее просмотренные»): чужие
# товары, их фото/цены не относятся к самой странице.
_REC_MARKERS = (
    ...
    'ранее просмотрен', 'недавно просмотрен', 'вы просматривали',
    'вы смотрели', 'недавно смотрели', 'просмотренные товар',
    'история просмотр',
)


def _html_bez_rec(c: _Ctx, orig: bool = False) -> str:
    """HTML страницы БЕЗ нижних блоков рекомендаций и «ранее просмотренных»
    (сквозные карусели с чужими товарами). Для проверки фото: заглушка «нет фото»
    у чужого товара из карусели не относится к самой странице.
    orig=True - исходный регистр (для alt=названий), иначе lower."""
    src = c.html if orig else c.html_lower
    rec = c.rec_html_full
    if rec and len(src) >= len(rec) and c.html_lower.endswith(rec):
        return src[:len(src) - len(rec)]
    # На не-product страницах rec-область отдельно не считается - находим первый
    # маркер карусели сами. Режем только если маркер в НИЖНЕЙ половине страницы
    # (карусели всегда внизу) - иначе случайное «рекомендуем» в тексте сверху
    # обрезало бы пол-страницы и мы пропустили бы реальные заглушки.
    total = len(c.html_lower)
    cut = total
    for m in _REC_MARKERS:
        i = c.html_lower.find(m)
        if total // 2 <= i < cut:
            cut = i
    return src[:cut]
...
def _d_cards_photos(c: _Ctx):
    ...
    # Нижние блоки рекомендаций / «ранее просмотренные» исключаем - там чужие
    # товары (сквозная карусель), их заглушки не относятся к этой странице.
    n = len(_NO_PHOTO_RE.findall(_html_bez_rec(c)))
...
BLOCK_DESCRIPTIONS = {
    ...
    'partners':      'Блок «Партнёры» на главной (класс/id partner… или заголовок «Партнёры»). Опционален по проекту.',
    ...
    'sort':          'Переключатель сортировки на листинге. Ищем не текст надписи (он бывает '
                      'на разных языках у гео-версий одного проекта), а разметку/URL: класс '
                      'виджета (catalog-sort/sort-item) или параметр ссылки ?sort=... - на '
                      'случай другой вёрстки запасной вариант - слова «сортировать»/«по '
                      'популярности» в видимом тексте. Необязательный пункт (не required) - '
                      'если сортировки на листинге правда нет, это отображается прочерком «-», '
                      'а не красным багом.',
    ...
    'reviews':       'Наличие отзывов на странице: микроразметка Schema.org Review или заголовок '
                      'блока («Отзывы клиентов/покупателей»). Проверка только на наличие - отзывов '
                      'нет не считается багом, показывается прочерком «-».',
    ...
    'rec_price':     'У товаров в нижних блоках есть видимая цена (₽ или «по запросу»). Нижний блок '
                      'разбивается на отдельные карточки по товарному контейнеру (СМУ: '
                      'catalog-product-card-item), у каждой настоящей карточки (со ссылкой и текстом, '
                      'не случайное совпадение класса в футере/скриптах) ищем цену. Если карточки снизу '
                      'есть, а цены не видно - баг (пустые цены снизу), с числом таких карточек. Блока '
                      'с карточками нет вообще (не на всех проектах есть по дизайну) - пункт '
                      'необязателен, прочерк «-».',
    'rec_links':     'Перелинковка: у каждой карточки в нижних блоках («Похожие товары» и т.п.) есть '
                      'настоящая ссылка на товар, а не декоративная плашка без href. Блок находится по '
                      'заголовкам-маркерам («похожие товар», «с этим покупают», «вам может понадоб…» и '
                      'т.п.), делится на отдельные карточки, у каждой с видимым текстом (не пустышка) '
                      'ищем href="/…" на товар. Хотя бы одна карточка без ссылки - баг с числом таких '
                      'карточек. Нижнего блока с карточками нет - пункт необязателен, прочерк «-».',
...
# Блок «Партнёры» на главной (опционален по проекту): класс/id partner…
# либо заголовок «Партнёры»/«Наши партнёры». Справочно, не баг.
_RE_PARTNERS = re.compile(
    r'(?:class|id)="[^"]*partner|<h[1-4][^>]*>[^<]*партн[её]р', re.I)


def _d_partners(c: _Ctx):
    return bool(_RE_PARTNERS.search(c.html_lower)), None
...
_MAIN_PROFILE = [
    *_HEADER,
    ...
    # Блок «Партнёры» - опционален по проекту, справочно (не баг).
    _b('partners', 'Партнёры',      False, _d_partners),
    *_FOOTER,
]
...
def check_content(html: str, type_code: str, css_hidden: tuple = (),
                  url: str = '', city: str = '') -> ContentResult:
    """
    ...
    for blk in _profile:
        ...
        if blk.key == 'photos':
            ...
            if not present and not _on_root:
                _photo_warn = True
                _photo_names = ', '.join(_photo_stub_names(_html_bez_rec(ctx, orig=True)))
