...
# Пороги «мелкого текста» для отчёта - те же, что в замере (console_run).
# Держим копией, чтобы report_priorities не тянул за собой браузерный модуль.
_FONT_SMALL_MIN = 3
_FONT_SMALL_SHARE = 0.2
        ...
        for w, m in vps.items():
            if (m.get('overflow') or 0) > 8:
                out.append(Finding(
                    ...
                    f'(горизонтальный скролл/обрезка)', url=url))
            if m.get('overlaps'):
                out.append(Finding(
                    'Предупреждение', 'Вёрстка',
                    f'на {w}px блоки накладываются друг на друга', url=url))
            # Мелкий шрифт: замер был и раньше, но в отчёт не выводился.
            # Ругаемся, только когда мелкого текста МНОГО - иначе сноска
            # или копирайт давали бы находку на каждой странице.
            всего, мелких = m.get('total') or 0, m.get('small') or 0
            if (мелких >= _FONT_SMALL_MIN and всего
                    and мелких > всего * _FONT_SMALL_SHARE):
                out.append(Finding(
                    'Предупреждение', 'Вёрстка',
                    f'на {w}px текст слишком мелкий', url=url,
                    detail=f'мелких элементов {мелких} из {всего}'
                           + (' · ' + '; '.join(m.get('small_examples') or [])
                              if m.get('small_examples') else ''),
                    fix_note='Норма: на ПК основной текст ~16px, на мобильных '
                             'не меньше 14px.'))
            if m.get('head_small'):
                out.append(Finding(
                    'Предупреждение', 'Вёрстка',
                    f'на {w}px заголовки слишком мелкие', url=url,
                    detail=f'{m["head_small"]} из {m.get("head_total") or 0} · '
                           + '; '.join(m.get('head_examples') or []),
                    fix_note='Норма: h1-h2 около 24px, h3-h6 18-22px на ПК.'))
            # Тач-таргеты меряются только на сенсорных ширинах.
            if m.get('touch_small') and (m.get('touch_total') or 0):
                _доля = m['touch_small'] / m['touch_total']
                if m['touch_small'] >= 3 and _доля > 0.2:
                    out.append(Finding(
                        'Предупреждение', 'Вёрстка',
                        f'на {w}px кнопки мельче 44x44 - неудобно нажимать '
                        f'пальцем', url=url,
                        detail=f'{m["touch_small"]} из {m["touch_total"]} · '
                               + '; '.join(m.get('touch_examples') or [])))
            # Меню на этой ширине: ни видимой навигации, ни бургера.
            if (m.get('menu_visible') is False
                    and m.get('menu_burger') is False):
                out.append(Finding(
                    'Ошибка', 'Вёрстка',
                    f'на {w}px меню недоступно - нет ни видимой навигации, '
                    f'ни кнопки-бургера', url=url))

        # ── Доступность: контраст, картинки, клавиатура ──
        a = p.get('a11y') or {}
        if (a.get('contrast_low') or 0) and (a.get('contrast_total') or 0):
            out.append(Finding(
                'Предупреждение', 'Вёрстка',
                'текст с низким контрастом (не проходит WCAG)', url=url,
                detail=f'{a["contrast_low"]} из {a["contrast_total"]} · '
                       + '; '.join(a.get('contrast_ex') or []),
                fix_note='Нужно 4.5:1 для обычного текста и 3:1 для крупного.'))
        for name in a.get('img_broken') or []:
            out.append(Finding('Ошибка', 'Изображения',
                               'картинка не загрузилась', url=url,
                               detail=name))
        if a.get('img_distorted'):
            out.append(Finding(
                'Предупреждение', 'Изображения',
                'картинка растянута - пропорции не совпадают с оригиналом',
                url=url, detail='; '.join(a['img_distorted'])))
        kb = a.get('keyboard') or {}
        if kb.get('steps'):
            if not kb.get('interactive'):
                out.append(Finding(
                    'Предупреждение', 'Вёрстка',
                    'навигация с клавиатуры: Tab не доходит до ссылок и кнопок',
                    url=url,
                    fix_note='Человек без мыши не сможет пользоваться сайтом.'))
            elif kb.get('no_focus_style') == kb['steps']:
                out.append(Finding(
                    'Предупреждение', 'Вёрстка',
                    'навигация с клавиатуры: фокус не виден', url=url,
                    detail='; '.join(kb.get('examples') or []),
                    fix_note='Добавить видимый стиль :focus (рамка или тень) - '
                             'иначе при обходе с клавиатуры непонятно, где вы.'))
    return out
...
def ssl_findings(ssl_check: Optional[dict]) -> list:
    """SSL-сертификаты хостов прогона. Сертификат один на хост, поэтому
    находка привязана к хосту, а не к странице."""
    out = []
    for h in (ssl_check or {}).get('hosts') or []:
        if h.get('error') or not h.get('available'):
            continue                 # не дозвонились - это не дефект сайта
        url = f'https://{h.get("host", "")}/'
        _срок = (f'действует до {(h.get("expires") or "")[:10]}'
                 if h.get('expires') else '')
        _изд = f' · выдан: {h["issuer"]}' if h.get('issuer') else ''
        for t in h.get('issues') or []:
            out.append(Finding('Ошибка', 'Безопасность', t, url=url,
                               detail=(_срок + _изд).strip(' ·')))
        for t in h.get('warnings') or []:
            out.append(Finding('Предупреждение', 'Безопасность', t, url=url,
                               detail=(_срок + _изд).strip(' ·')))
    return out


def _speed_findings(r) -> list:
    """Время ответа документа (чек-лист: до 1с - норма, до 2.5с -
    предупреждение, дальше - чинить).

    Раньше скорость жила только колонкой на листе «Страницы» и тегом
    «разработка» - отдельной строкой в «Проблемы» не попадала, и найти
    медленные страницы фильтром было нельзя.
    """
    rating = getattr(r, 'speed_rating', None)
    if not rating or rating == 'fast':
        return []
    ms = getattr(r, 'elapsed_ms', 0) or 0
    сек = f'{ms / 1000:.1f} с'
    if rating == 'normal':
        return [Finding(
            'Предупреждение', 'Валидация и скорость',
            'страница отвечает дольше 1 секунды',
            r.city, r.type_label, r.url,
            detail=f'время ответа: {сек}',
            fix_note='До 1 секунды - норма, до 2.5 - терпимо. Дальше растёт '
                     'отказ покупателя и падает лимит обхода у робота.')]
    текст = ('сервер отвечает очень долго' if rating == 'very_slow'
             else 'страница отвечает дольше 2.5 секунд')
    return [Finding(
        'Ошибка', 'Валидация и скорость', текст,
        r.city, r.type_label, r.url,
        detail=f'время ответа: {сек}',
        fix_note='Ускорить ответ документа: кеш, сжатие, запросы к базе. '
                 'Норма - до 1 секунды.')]
...
def _link_findings(bl, *, city='', page_type='', url='') -> list:
    ...
    for b in bl.get('forbidden') or []:
        out.append(Finding(
            'Предупреждение', SEC_LINKS,
            f'внутренняя ссылка ведёт на закрытую страницу ({b.get("code")})',
            city, page_type, b.get('url', ''),
            detail=f'страница со ссылкой: {url}',
            fix_note='Свой сайт не должен закрывать страницу, на которую сам '
                     'ссылается: проверьте права или уберите ссылку. Бывает и '
                     'от защиты от ботов на своём WAF/прокси - тогда это '
                     'ограничение проверки, а не дефект.'))
    for b in bl.get('ext_broken') or []:
        out.append(Finding('Предупреждение', SEC_LINKS,
                           ...
                           city, page_type, b.get('url', ''),
                           detail=f'страница со ссылкой: {url}'))
    for b in bl.get('ext_forbidden') or []:
        out.append(Finding(
            'Предупреждение', SEC_LINKS,
            f'внешняя ссылка ведёт на закрытую страницу ({b.get("code")})',
            city, page_type, b.get('url', ''),
            detail=f'страница со ссылкой: {url}',
            fix_note='Проверьте ссылку вручную: у чужого сайта 403 чаще всего '
                     'отдаёт защита от ботов, а не закрытая страница.'))
...
_RULES = [
    ...
    # Сам файл robots.txt (вес/формат/кодировка/комментарии/Clean-Param).
    ('Индексация', 'robots.txt тяжелее', 1, 'SEO + разработка', 'robots_file_hygiene',
     'Починить сам файл robots.txt',
     'За пределом 500 КБ робот перестаёт читать файл - правила ниже не работают.'),
    ('Индексация', 'вместо robots.txt', 1, 'Разработка', 'robots_file_hygiene',
     'Починить сам файл robots.txt',
     'Робот получает HTML-страницу вместо правил - значит правил у сайта нет.'),
    ('Индексация', 'robots.txt начинается с bom', 1, 'Разработка', 'robots_file_hygiene',
     'Починить сам файл robots.txt',
     'С BOM первая директива не читается - пропадает целая группа правил.'),
    ('Индексация', 'robots.txt не в кодировке', 1, 'Разработка', 'robots_file_hygiene',
     'Починить сам файл robots.txt',
     'Часть правил робот прочтёт неверно.'),
    ('Индексация', 'robots.txt отдаётся не как текст', 3, 'Разработка',
     'robots_file_hygiene', 'Починить сам файл robots.txt',
     'Ожидается text/plain - иначе часть роботов файл проигнорирует.'),
    ('Индексация', 'закомментирована', 2, 'SEO', 'robots_commented',
     'Разобрать закомментированные директивы robots.txt',
     'Правило выключено знаком «#», а выглядит рабочим - легко пропустить.'),
    ('Индексация', 'clean-param', 3, 'SEO', 'robots_clean_param',
     'Прописать Clean-Param в robots.txt',
     'Без неё Яндекс держит в индексе копии страниц с метками (utm_source, sort).'),
    # Карта сайта: формат/кодировка и прозвон адресов.
    ('Индексация', 'карта сайта не в формате', 1, 'Разработка', 'sitemap_format',
     'Починить формат и кодировку карты сайта',
     'Карта не в .xml/.txt - робот не прочитает её вовсе.'),
    ('Индексация', 'кодировка карты сайта', 1, 'Разработка', 'sitemap_format',
     'Починить формат и кодировку карты сайта',
     'Робот пойдёт по заявленной кодировке и прочитает адреса неверно.'),
    ('Индексация', 'отвечает не 200', 2, 'Разработка', 'sitemap_bad_status',
     'Убрать из карты сайта адреса, не отдающие 200',
     'Редиректы и ошибки в карте тратят лимит обхода.'),
    ('Индексация', 'закрыт noindex', 2, 'SEO', 'sitemap_noindex',
     'Развести карту сайта и noindex',
     'Карта говорит «в индекс», страница - «не индексировать»: сигналы спорят.'),
    ('Индексация', 'не удалось проверить', 3, 'SEO', 'sitemap_unreachable',
     'Открыть вручную адреса, до которых не дозвонились',
     'До адреса не доехали мы: это ограничение проверки, а не подтверждённый дефект.'),
    # Формат адресов - новые виды.
    ('Индексация', 'домен указан в адресе дважды', 2, 'Разработка', 'url_format_domain',
     'Починить шаблон ссылок с задвоенным доменом',
     'В путь попал домен или схема - адрес нерабочий и плодит дубли.'),
    ('Индексация', 'длиннее 2048', 3, 'SEO + разработка', 'url_format_long',
     'Сократить слишком длинные адреса',
     'За пределом 2048 символов адрес обрезается в чужих системах.'),
    ...
    # Время ответа самого документа (не ресурсов). Порядок важен: «дольше
    # 1 секунды» должно проверяться ПОСЛЕ «дольше 2.5», иначе подстрока
    # перехватит обе находки.
    ('Валидация и скорость', 'отвечает очень долго', 1, 'Разработка', 'page_speed_slow',
     'Ускорить ответ сервера',
     'Долгий ответ теряет покупателя ещё до отрисовки страницы.'),
    ('Валидация и скорость', 'дольше 2.5 секунд', 2, 'Разработка', 'page_speed_slow',
     'Ускорить ответ сервера',
     'После 2.5 секунд заметно растёт доля отказов, а робот реже обходит сайт.'),
    ('Валидация и скорость', 'дольше 1 секунды', 3, 'Разработка', 'page_speed_warn',
     'Подтянуть время ответа до 1 секунды',
     'Норма по чек-листу - до 1 секунды; сейчас страница отвечает дольше.'),
    ...
    ('Метаданные', 'частотном порядке слов', 2, 'SEO', 'title_key_order',
     'Поставить ключ в title в самом частотном порядке слов',
     'Порядок слов меняет запрос: «труба бесшовная» и «бесшовная труба» ищут '
     'по-разному, и title должен начинаться с той версии, которую ищут чаще.'),
    ...
    (SEC_LINKS, 'на закрытую страницу', 2, 'Разработка', 'links_forbidden',
     'Разобраться со ссылками на закрытые страницы (403)',
     'Ссылка ведёт туда, куда нет доступа: либо закрытый раздел попал в '
     'навигацию, либо права выданы неверно.'),
    ...
    ('Изображения', 'не загрузилась', 1, 'Разработка', 'img_broken',
     'Починить битые картинки',
     'Пустые места вместо фото снижают доверие и конверсию.'),
    ('Изображения', 'растянута', 2, 'Разработка', 'img_distorted',
     'Починить пропорции картинок',
     'Растянутое фото выглядит небрежно и портит впечатление о товаре.'),
    ...
    # Отзывы на карточке: сначала «нет блока», потом качество разметки.
    ('Разметка', 'нет блока отзывов', 2, 'Контент', 'reviews_missing',
     'Добавить блок отзывов на карточку товара',
     'Отзывы помогают выбрать товар, а разметка даёт звёзды в сниппете.'),
    ('Разметка', 'блок отзывов на товаре не размечен', 2, 'Разработка',
     'reviews_markup', 'Разметить отзывы на товаре (schema.org/Review)',
     'Без разметки поиск не покажет звёзды - блок работает только на сайте.'),
    ('Разметка', 'накруткой звёзд', 1, 'Разработка', 'reviews_fake_rating',
     'Убрать рейтинг без отзывов или добавить сами отзывы',
     'Сводная оценка без единого отзыва - для Google накрутка: сниппет со '
     'звёздами снимут, возможны ручные санкции.'),
    ('Разметка', 'сводной оценки', 3, 'Разработка', 'reviews_markup',
     'Разметить отзывы на товаре (schema.org/Review)',
     'AggregateRating нужен, чтобы в выдаче появилась средняя оценка.'),
    ('Разметка', 'отзыв', 2, 'Контент', 'reviews_quality',
     'Привести отзывы в правдоподобный вид',
     'Одна дата, один автор и одинаковые оценки у всех - явная разовая '
     'генерация: за такую разметку поисковики дают ручные санкции.'),
    # OpenGraph: отдельно «тега нет» и «тег есть, но значение негодное» -
    # чинят это разные вещи (шаблон против подстановки переменной).
    ('Разметка', 'нет opengraph-тега', 2, 'Разработка', 'og_missing',
     'Добавить OpenGraph-теги',
     'Без них соцсети и мессенджеры соберут анонс ссылки как попало.'),
    ('Разметка', 'пустой', 2, 'Разработка', 'og_empty',
     'Заполнить пустые OpenGraph-теги',
     'Тег есть, но значение не подставилось - для анонса это то же, что тега нет.'),
    ('Разметка', 'og:url', 2, 'Разработка', 'og_url',
     'Поправить og:url',
     'В анонсе ссылки соцсеть покажет чужой или битый адрес.'),
    ('Разметка', 'og:image', 2, 'Разработка', 'og_image',
     'Поправить картинку анонса (og:image)',
     'Без пригодной картинки ссылка в соцсетях выглядит пустой и хуже кликается.'),
    ('Разметка', 'og:description', 3, 'SEO', 'og_desc',
     'Сократить og:description',
     'Длинное описание обрезается в анонсе на полуслове.'),
    ('Разметка', 'og:type', 3, 'SEO', 'og_type',
     'Проверить og:type',
     'Тип контента подсказывает соцсети, как показать анонс.'),
    ...
    # SSL - раньше заголовков: истёкший сертификат закрывает сайт целиком.
    ('Безопасность', 'ssl-сертификат истёк', 1, 'Разработка', 'ssl_cert',
     'Продлить SSL-сертификат',
     'Браузер показывает предупреждение вместо сайта - трафик теряется весь.'),
    ('Безопасность', 'ssl-сертификат истекает', 1, 'Разработка', 'ssl_cert',
     'Продлить SSL-сертификат',
     'До истечения считанные дни: продлевать надо сейчас, а не после сбоя.'),
    ('Безопасность', 'выписан на другой домен', 1, 'Разработка', 'ssl_cert',
     'Выпустить SSL-сертификат на нужный домен',
     'Браузер не считает соединение доверенным и пугает посетителя.'),
    ('Безопасность', 'не проходит проверку', 1, 'Разработка', 'ssl_cert',
     'Починить SSL-сертификат',
     'Сертификату не доверяют - для посетителя сайт выглядит небезопасным.'),
    ('Безопасность', 'неполную цепочку', 2, 'Разработка', 'ssl_chain',
     'Отдавать полную цепочку SSL',
     'Десктоп обычно дотягивает недостающее сам, а часть мобильных и роботов - нет.'),
    ('Безопасность', 'ещё не вступил в силу', 2, 'Разработка', 'ssl_cert',
     'Проверить дату начала SSL-сертификата',
     'Сертификат из будущего браузеры не принимают.'),
    ...
    ('Вёрстка', 'теги-артефакты', 3, 'Разработка', 'layout_artifacts',
     'Убрать теги-артефакты из подзаголовков, шапки и подвала',
     'Жирность и курсив в меню и заголовках задаются стилями. Теги <strong>/'
     '<b>/<em> там - следы копипаста из редактора: сбивают семантику и мешают '
     'единому оформлению.'),
    ('Вёрстка', 'не прямыми ссылками', 2, 'Разработка', 'menu_direct_links',
     'Сделать пункты меню прямыми ссылками',
     'По href="#" и javascript: краулер не пройдёт - разделы остаются вне индекса.'),
    ('Вёрстка', 'ссылок на категории каталога', 2, 'SEO + разработка',
     'menu_catalog_link', 'Добавить в меню ссылки на категории каталога',
     'Категории недоступны в один клик - страдают и навигация, и перелинковка.'),
    ('Вёрстка', 'меню не проверено', 3, 'SEO', 'menu_unchecked',
     'Проверить меню вручную',
     'Разметку меню автоматически распознать не удалось - убедиться глазами, '
     'что пункты - прямые ссылки и категории доступны из шапки.'),
    ('Вёрстка', 'меню недоступно', 1, 'Разработка', 'adaptive_menu',
     'Вернуть меню на всех разрешениях',
     'На этой ширине посетитель не может открыть навигацию вообще.'),
    ('Вёрстка', 'текст слишком мелкий', 2, 'Разработка', 'font_small',
     'Увеличить шрифт до читаемого',
     'Мелкий текст плохо читается, особенно с телефона - растут отказы.'),
    ('Вёрстка', 'заголовки слишком мелкие', 3, 'Разработка', 'font_small_head',
     'Привести размеры заголовков к норме',
     'Заголовки задают структуру страницы - они должны выделяться размером.'),
    ('Вёрстка', 'мельче 44x44', 2, 'Разработка', 'touch_targets',
     'Увеличить кнопки под палец',
     'В мелкую кнопку на телефоне не попасть - теряются клики и заявки.'),
    ('Вёрстка', 'низким контрастом', 2, 'Разработка', 'a11y_contrast',
     'Поднять контраст текста',
     'Бледный текст не читают ни люди со слабым зрением, ни на солнце.'),
    ('Вёрстка', 'навигация с клавиатуры', 3, 'Разработка', 'a11y_keyboard',
     'Починить навигацию с клавиатуры',
     'Без мыши сайтом пользуются и люди с ограничениями, и те, кому так быстрее.'),
    ...
    ('Я.Бизнес', 'не зарегистрирован', 1, 'SEO', 'yb_missing_org',
     'Завести карточки организаций на города поддоменов',
     'Без карточки город не участвует в локальной выдаче и на картах - теряются '
     'звонки и маршруты.'),
    ('Я.Бизнес', 'не объединены в сеть', 2, 'SEO', 'yb_chain',
     'Свести филиалы в Сеть',
     'Разрозненные карточки хуже управляются и путают рейтинг филиалов.'),
    ('Я.Бизнес', 'не заполнено', 3, 'SEO', 'yb_profile',
     'Дозаполнить профили организаций',
     'Чем полнее карточка, тем выше она в локальной выдаче и тем больше '
     'обращений с карт.'),
    ('Я.Бизнес', 'поддомена под этот город нет', 3, 'SEO', 'yb_orphan',
     'Разобрать карточки без поддомена',
     'Либо город стоит открыть на сайте, либо карточка в аккаунте лишняя.'),
    ('Я.Бизнес', '', 3, 'SEO', 'yb_generic',
     'Проверить карточки Яндекс.Бизнеса',
     'Карточки организаций дают трафик и звонки с карт.'),

    ('Отзывы', 'не удалось получить', 3, 'SEO', 'reviews_no_data',
     'Починить сбор рейтингов филиалов',
     'Без рейтинга нельзя понять, каким филиалам нужны отзывы в первую очередь.'),
    ('Отзывы', 'ни одного отзыва', 2, 'SEO', 'reviews_empty',
     'Набрать первые отзывы филиалам без отзывов',
     'Карточка без единого отзыва вызывает меньше доверия, чем карточка с оценкой.'),
    ('Отзывы', 'рейтинг не выставлен', 3, 'SEO', 'reviews_no_rating',
     'Добрать отзывы с оценкой',
     'Отзывы без звёзд не формируют рейтинг - карточка выглядит неоценённой.'),
    ('Отзывы', '', 2, 'SEO', 'reviews_buy',
     'Докупить отзывы филиалам с низким рейтингом',
     'Рейтинг ниже 4.7 заметно снижает число обращений с карт и доверие покупателя.'),
...
def title_key_order_findings(title_key: Optional[dict]) -> list:
    """Ключ в title стоит НЕ в самой частотной последовательности слов
    (пункт ручной проработки меты). Данные - title_key_order.run_check."""
    if not title_key or not title_key.get('available'):
        return []
    out = []
    for p in title_key.get('pages') or []:
        if p.get('status') != 'bad':
            continue
        out.append(Finding(
            'Ошибка', 'Метаданные',
            'в title ключ стоит не в самом частотном порядке слов',
            p.get('city', ''), url=p.get('url', ''),
            detail=p.get('detail', ''),
            fix_note=f'«{p.get("свой", "")}» - {p.get("свой_вес", 0)}; '
                     f'«{p.get("лучший", "")}» - {p.get("лучший_вес", 0)} '
                     f'(частотность {p.get("тип", "")}, все регионы)'))
    return out
...
# Сколько символов адреса показываем в ячейке отчёта: длиннее - обрезаем,
# полную длину пишем в «Подробности». Иначе строка с адресом на 2000+ символов
# растягивает лист и её невозможно прочитать.
_MAX_URL_SHOWN = 300
...
def url_format_findings(indexing_summary: Optional[dict]) -> list:
    ...
    for kind, text, как in _URL_FORMAT_KINDS:
        ...
        пути = uf.get(kind) or []
        for p in пути:
            # Адрес показываем обрезанным: находка «длиннее 2048 символов» иначе
            # вываливает в ячейку весь двухтысячный адрес, и строку не прочитать.
            _полный = _idx_url(indexing_summary, p)
            _показ = (_полный if len(_полный) <= _MAX_URL_SHOWN
                      else _полный[:_MAX_URL_SHOWN] + '…')
            _хвост = (f' · длина адреса: {len(_полный)} символов'
                      if len(_полный) > _MAX_URL_SHOWN else '')
            out.append(Finding(
                'Предупреждение', 'Индексация', text, url=_показ,
                detail=f'таких адресов: {всего}{_хвост}',
...
_MAPS_ORG = 'https://yandex.ru/maps/org/'


def yabusiness_findings(yabusiness: Optional[dict]) -> list:
    """Я.Бизнес (кабинет Справочника): поддомен без карточки под свой город,
    карточки без поддомена, филиалы не сведены в Сеть и незаполненные поля
    профиля. Раньше всё это было только на листе «Я.Бизнес и GMB» и ни в
    «Проблемы», ни в «План работ» не попадало. ✅-списки (поддомены с
    организацией) не переносим - это справка, а не находка."""
    yb = yabusiness or {}
    if not yb.get('available'):
        return []
    out = []

    for m in yb.get('missing') or []:
        out.append(Finding(
            'Ошибка', 'Я.Бизнес',
            'поддомен города не зарегистрирован в Яндекс.Бизнесе',
            city=m.get('city', ''), url=m.get('url', ''),
            detail='в аккаунте нет карточки организации под этот город',
            fix_note='Завести карточку организации на свой город - без неё '
                     'поддомен не участвует в локальной выдаче и на картах.'))

    for o in yb.get('orphan_orgs') or []:
        out.append(Finding(
            'Предупреждение', 'Я.Бизнес',
            'карточка организации есть, а поддомена под этот город нет',
            city=o.get('city', ''),
            url=_MAPS_ORG + str(o.get('permalink', '')),
            detail=f'регион карточки: {o.get("region") or "не указан"}'
                   + (f' · адрес: {o["addr"]}' if o.get('addr') else ''),
            fix_note='Либо открыть поддомен под этот город, либо убрать '
                     'лишнюю карточку из аккаунта.'))

    cch = yb.get('chain_check') or {}
    if cch and not cch.get('united'):
        города = ', '.join(
            str(s.get('city') or s.get('permalink', ''))
            for s in (cch.get('standalone_list') or [])[:10])
        out.append(Finding(
            'Ошибка', 'Я.Бизнес',
            'филиалы не объединены в Сеть - часть карточек стоит отдельно',
            detail=f'вне сети: {cch.get("standalone_companies", 0)} карточек'
                   + (f' ({города})' if города else ''),
            fix_note=f'Свести отдельные карточки в Сеть. Сейчас сетей: '
                     f'{cch.get("chains", 0)}, филиалов в них: '
                     f'{cch.get("chain_members", 0)}.'))

    for o in (yb.get('profile_check') or {}).get('orgs') or []:
        # Строка на КАЖДОЕ незаполненное поле: так задачу видно поимённо и
        # можно раздать по полям, а не «доработать профиль вообще».
        for поле in o.get('missing') or []:
            out.append(Finding(
                'Предупреждение', 'Я.Бизнес',
                f'в профиле организации не заполнено: {поле}',
                city=o.get('city', ''),
                url=_MAPS_ORG + str(o.get('permalink', '')),
                fix_note=f'Заполнить поле в карточке Яндекс.Бизнеса. Сейчас '
                         f'заполнено {o.get("filled", 0)} из '
                         f'{o.get("total", 0)} полей.'))

    return out


def review_priority_findings(review_priority: Optional[dict]) -> list:
    """Отзывы: филиалы с низким рейтингом (худший из Яндекс/2ГИС). Сам план
    докупки (сколько отзывов и в каком порядке) остаётся таблицей на листе
    «Я.Бизнес и GMB» - в «Проблемы» он не ложится, там колонки под страницу."""
    rp = review_priority or {}
    if not rp.get('available'):
        return []
    out = []
    for b in rp.get('branches') or []:
        y, g = b.get('yandex') or {}, b.get('twogis') or {}
        оценки = []
        if y.get('rating') is not None:
            оценки.append(f'Яндекс {y["rating"]} ({y.get("count", 0)} отз.)')
        if g.get('rating') is not None:
            оценки.append(f'2ГИС {g["rating"]} ({g.get("count", 0)} отз.)')
        ссылка = b.get('yandex_url') or b.get('2gis_url') or ''
        рейтинг = b.get('rating')

        if рейтинг is None:
            # Рейтинга нет по двум разным причинам - и это разные задачи.
            if b.get('no_reviews'):
                out.append(Finding(
                    'Предупреждение', 'Отзывы',
                    'у филиала нет ни одного отзыва',
                    city=b.get('city', ''), url=ссылка,
                    detail='карточка открывается, отзывов 0',
                    fix_note='Начать с этого филиала: карточка без отзывов '
                             'вызывает меньше доверия, чем вообще без карточки.'))
                continue
            if b.get('no_rating_yet'):
                out.append(Finding(
                    'Предупреждение', 'Отзывы',
                    'у филиала есть отзывы, но рейтинг не выставлен',
                    city=b.get('city', ''), url=ссылка,
                    detail='; '.join(оценки) or 'отзывы без оценок',
                    fix_note='Отзывы оставлены текстом без звёзд либо их пока '
                             'мало для расчёта - докупить с оценкой.'))
                continue
            # Ни один источник не ответил - про рейтинг мы ничего не знаем,
            # выдавать это за низкий рейтинг нельзя.
            out.append(Finding(
                'Предупреждение', 'Отзывы',
                'не удалось получить рейтинг филиала',
                city=b.get('city', ''), url=ссылка,
                detail='; '.join(оценки) or 'нет данных ни в Яндексе, ни в 2ГИС',
                fix_note='Проверить ссылки на карточки: колонки yandex_map_url '
                         'и twogis_map_url в карте присутствия проекта.'))
            continue
        if not b.get('low_rating'):
            continue
        out.append(Finding(
            'Предупреждение', 'Отзывы',
            f'рейтинг филиала ниже 4.7 - {рейтинг}',
            city=b.get('city', ''), url=ссылка,
            detail='; '.join(оценки),
            fix_note=f'Докупить отзывы: {b.get("order", 2)} шт. '
                     + ('Рейтинг критично низкий - брать в работу первым.'
                        if b.get('negative') else
                        'Порядок докупки - в таблице на листе «Я.Бизнес и GMB».')))
    return out
