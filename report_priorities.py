...
def review_priority_findings(review_priority: Optional[dict]) -> list:
    ...
    for b in rp.get('branches') or []:
        ...
        if рейтинг is None:
            # Рейтинг не получен (нет ссылки в конфиге/карточка не открылась) -
            # это не низкий рейтинг, а непроверенный филиал: смешивать нельзя.
            out.append(Finding(
                ...
                fix_note='Проверить ссылки на карточки в '
                         'catalogs/reviews-<проект>.csv.'))
            continue
