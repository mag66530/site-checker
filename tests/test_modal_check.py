...
def test_уже_не_видна_до_проверки():
    # Окно исчезло само сразу после отправки, и попапа-подтверждения на странице
    # нет. Раз окно исчезает после отправки - закрытие РАБОТАЕТ → «Да» (✓).
    # Не пугаем жёлтым «Проверить» на ровном месте (минимизируем ⚠).
    state = {"visible": False}
    modal = _FakeModal(state)
    page = _FakePage(state)
    статус, способ = t._проба_закрытия_модалки(page, modal)
    assert статус == "Да"
    assert "закрылось само" in способ
