...
# Цифры координат SVG-иконок в ТЕЛЕ страницы (>24 КБ выше подвала). Извлечение
# телефонов ограничено шапкой+подвалом, поэтому такие цифры туда не попадают.
_BODY_FILLER = ('<div class="card"><svg viewBox="0 0 48 48"><path '
                'd="M8.22405 38.7432 10.7502 40.9283 13.9458 42.2673 22.0207 '
                '21.96"/></svg>Мепен - металлопрокат</div>') * 300  # ~40 КБ


def test_svg_body_coords_not_phone_belgorod():
    """Кейс Белгорода (mepen): в отчёте всплывал фантомный «+7 (945) 544-08-80»,
    которого на сайте НЕТ (заказчик не смог найти его вручную). Он собирался
    старым кодом из разрозненных цифр path-координат SVG-иконок (d="…13.9458…"),
    лежащих В СЕРЕДИНЕ документа. Телефоны берём только из шапки+подвала - цифры
    из тела страницы номером не считаются. На сайте виден лишь общий номер."""
    # Фантомные цифры (9455440880) стоят в ТЕЛЕ, сразу под шапкой, а дальше ~40 КБ
    # разметки до <footer> - значит они и не в шапке, и дальше 24 КБ-«хвоста» перед
    # подвалом, т.е. вне обоих регионов (как SVG-координаты на реальной странице).
    html = ('<header><a href="tel:+74722770686">+7 (472) 277-06-86</a></header>'
            '<main><svg><path d="M9455440880 1.2 3.4"/></svg>' + _BODY_FILLER +
            '</main><footer>© 2026 Мепен, Белгород</footer>')
    c = kp.extract_site_contacts(html)
    assert c["phones"] == ["4722770686"], c["phones"]    # только реальный номер
    assert "9455440880" not in c["phones"]


def test_belgorod_empty_seo_shows_dash_not_phantom():
    """Тот же кейс на уровне check_variables: в КП Белгорода отдельного SEO-номера
    НЕТ (пустая ячейка), на сайте - только общий +7 (472) 277-06-86. Значит:
      • «Тел. Общий Город» → ✓ (совпадает с КП);
      • «Тел. SEO Город»   → «–» (отдельного номера в КП нет, на сайте общий),
        а НЕ ✗ с фантомным номером из цифр SVG в теле страницы."""
    row = kp.KPRow(domain="belgorod.mepen.ru", city="Белгород",
                   phone_seo="", phone_ad="", phone_common="7 (472) 277-06-86",
                   all_phones="4722770686", email="belgorod@mepen.ru",
                   address="Корочанская улица, 409А", country="Россия")
    html = ('<header><a href="tel:+74722770686">+7 (472) 277-06-86</a>'
            '<a href="mailto:belgorod@mepen.ru">belgorod@mepen.ru</a></header>'
            '<main><svg><path d="M9455440880 1.2 3.4"/></svg>' + _BODY_FILLER +
            '</main><footer>© 2026 Мепен, Белгород</footer>')
    r = kp.check_variables(html, "belgorod.mepen.ru", row=row)
    by = {f["field"]: f for f in r["fields"]}
    assert by["Тел. Общий Город"]["status"] == "ok"
    assert by["Тел. Общий Город"]["found"] == "+7 (472) 277-06-86"
...
def test_site_address_not_fooled_by_city_picker_popup():
    """Текст попапа выбора города («Ваш город: Ульяновск? Всё верно. Выбрать
    другой город …») НЕ должен попадать в «Сайт» как адрес - даже если рядом
    есть цифра (телефон). Берём адрес только по метке «Адрес:» с реальной
    улицей/домом."""
    junk = ('<main>Ваш город: Ульяновск? Всё верно Выбрать другой город '
            'Ульяновск +7 (842) 271-36-69</main>')
    assert kp._site_address_full(junk) == ""
