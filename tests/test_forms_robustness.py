...
def test_обработка_ошибок_не_ajax_формы():
    # Обычная (не AJAX) форма отправляется переходом страницы - ошибку рисует
    # сервер, оборвать её проба не может. Раньше такие формы шли прочерком
    # («почему прочерк, надо проверять») - теперь честный ⚠ с инструкцией.
    sym, ком = t._матрица_классифицировать('Обработка ошибок',
                                           'не проверить автоматически')
    assert sym == '⚠'
    assert 'вручную' in ком.lower()
    # Контроль: рабочие исходы не поехали.
