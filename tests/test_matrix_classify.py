...
def test_кнопка_всегда_активная_крест():
    # По требованию: кнопку можно нажать на пустой форме = дефект (✗), не ⚠.
...
def test_защита_спама_бинарно():
    # По требованию - бинарно, минимум ⚠: все повторы прошли = лимита нет (✗);
    # повтор заблокирован = защита есть (✓).
    assert _sym("Защита от спама (активно)", "Не сработала") == "✗"
    assert _sym("Защита от спама (активно)", "Блокирует повтор") == "✓"
    assert _sym("Защита от спама (активно)", "Сработала") == "✓"
...
def test_матрица_рендерит_модалку_галочкой(tmp_path):
    ...
    assert got.get("Защита от спама (активно)") == "✓"  # «блокирует повтор» → защита есть (бинарно, без ⚠)
...
def test_матрица_двойная_отправка_не_защищена_крест(tmp_path):
    from openpyxl import load_workbook
    old = t.MATRIX_DETAILS_FILE
    t.MATRIX_DETAILS_FILE = str(tmp_path / "matrix_details.json")
    try:
        path = tmp_path / "log_forms.xlsx"
        _лог_с_формой(path, {"Двойная отправка": "не защищена"})
        t.построить_матрицу_проверок(str(path))
        ws = load_workbook(str(path))["Москва"]
        assert str([ws.cell(i, 2).value for i in range(2, ws.max_row + 1)
                    if str(ws.cell(i, 1).value) == "Двойная отправка"][0]) == "✗"
...
def test_матрица_показывает_конкретную_причину_стилизации(tmp_path):
    # «разнобой» ✗: в ячейке - КОНКРЕТНО какие поля выбиваются и эталонный стиль,
    # а не общая фраза «поля различаются - похоже на баг вёрстки».
    from openpyxl import load_workbook
    old = t.MATRIX_DETAILS_FILE
    t.MATRIX_DETAILS_FILE = str(tmp_path / "matrix_details.json")
    try:
        path = tmp_path / "log_forms.xlsx"
        _лог_с_формой(path, {"Стилизация полей": "разнобой"})
        деталь = ("поля различаются по стилю (шрифт/размер/рамка/радиус): "
                  "выбиваются поля [3] из 3. Эталон (поле 1): шрифт Inter 16px, "
                  "рамка 1px solid #FFEAE8, радиус 6px. кнопка дефолтная")
        t.record_matrix_detail("Москва", "Товар", "Купить в один клик",
                               "Стилизация полей", деталь)
        t.построить_матрицу_проверок(str(path))
        ws = load_workbook(str(path))["Москва"]
        assert str([ws.cell(i, 2).value for i in range(2, ws.max_row + 1)
                    if str(ws.cell(i, 1).value) == "Стилизация полей"][0]) == "✗"
        got = _коммент_ячейки(ws, "Стилизация полей")
        assert got and "выбиваются поля [3]" in got and "Эталон" in got
        assert "похоже на баг вёрстки" not in got            # общая фраза заменена
    finally:
        t.MATRIX_DETAILS_FILE = old
...
def test_нет_детали_остаётся_общая_фраза_правила(tmp_path):
    ...
    try:
        ...
        assert got and "не блокируется" in got  # общая фраза правила (✗ «не защищена»)
    finally:
        t.MATRIX_DETAILS_FILE = old
...
def test_без_согласия_три_состояния_без_ложного_креста():
    # «да» → ✓ (обязательность подтверждена)
    assert t._матрица_классифицировать("Без согласия не отправить", "да")[0] == "✓"
    # «нет» → ✗ (чекбокса согласия вообще нет - отправить без согласия можно)
    sym_no, cm_no = t._матрица_классифицировать("Без согласия не отправить", "нет")
    assert sym_no == "✗" and cm_no
    # «не подтверждено» → ⚠, НЕ ✗: чекбокс есть, но обязательность не удалось
    # подтвердить автоматически (JS-проверка согласия). Это и был ложный ✗ на
    # форме «Контакты» - теперь честный ⚠, а не ложная ошибка.
    sym_uc, cm_uc = t._матрица_классифицировать("Без согласия не отправить", "не подтверждено")
    assert sym_uc == "⚠"
    assert cm_uc and "вручную" in cm_uc.lower()
    # пусто (согласие текстом) → «–»
...
def test_согласие_текстом_у_кнопки_крестик_с_комментом():
    # Согласие ТЕКСТОМ у кнопки (без чекбокса) - это НЕ полноценный чекбокс:
    ...
    assert t.текст_подразумевает_согласие(
        ...
        "персональных данных ... N 152-ФЗ «О персональных данных»") is True
    # Формулировка «политикой конфиденциальности» (случай МПЭ «Рассчитать
    # стоимость доставки») теперь тоже распознаётся как текст-согласие.
    assert t.текст_подразумевает_согласие(
        "Нажимая на кнопку «Отправить заявку», вы соглашаетесь с нашей "
        "политикой конфиденциальности.") is True
    assert t.текст_подразумевает_согласие("Имя Телефон Отправить") is False
    # Голая ссылка «Политика конфиденциальности» без активного глагола согласия
    # - это НЕ текст-согласие.
    assert t.текст_подразумевает_согласие("Политика конфиденциальности") is False

    sym, cm = t._матрица_классифицировать("Наличие чек боксов согласия",
                                          "0 - согласие текстом у кнопки")
    assert sym == "✗"
    assert cm and "текстом" in cm.lower()
    # обычные случаи (0 / ≥2 реальных чекбокса) не сломаны
    assert t._матрица_классифицировать("Наличие чек боксов согласия", "0 (нужно ≥2)")[0] == "✗"
    assert t._матрица_классифицировать("Наличие чек боксов согласия", "2 (нужно ≥2)")[0] == "✓"


# ── XSS второй (тестовой) заявкой: отдельная строка сводится в форму ──
def test_xss_отдельной_строкой_сводится_в_форму_и_матрицу(tmp_path):
    # XSS теперь проверяется ВТОРОЙ (тестовой) заявкой: основная уходит с чистым
    # именем (для честного подтверждения), а вердикт XSS пишется ОТДЕЛЬНОЙ строкой
    # «Защита от XSS: <форма>» (как серверная валидация). Гарантируем, что
    # консолидация сводит её в ту же форму, а матрица показывает верный символ -
    # результат XSS не теряется при выносе во вторую заявку.
    from openpyxl import Workbook, load_workbook
    for i, (вердикт, символ) in enumerate(
            (("Защищена", "✓"), ("УЯЗВИМА", "✗"), ("Проверить", "⚠"))):
        path = tmp_path / f"log_xss_{i}.xlsx"
        wb = Workbook(); sv = wb.active; sv.title = "Сводка"
        ws = wb.create_sheet("Логи"); ws.append(list(t.LOG_HEADERS))

        def _row(название, **kw):
            r = {h: "" for h in t.LOG_HEADERS}
            r.update({"Город": "Москва", "Страница": "Главная",
                      "URL": "https://example.ru/", "Название": название,
                      "Где находится": "Главная"})
            r.update(kw)
            return [r[h] for h in t.LOG_HEADERS]

        # основная (первая) заявка - чистое имя, XSS-колонка ПУСТАЯ
        ws.append(_row("Заказать звонок",
                       Статус="УСПЕШНО (Playwright - как ручная отправка)"))
        # вторая (тестовая) заявка - вердикт XSS отдельной строкой «: <форма>»
        ws.append(_row("Защита от XSS: Заказать звонок",
                       Статус="OK", **{"Защита от XSS": вердикт}))
        wb.save(str(path))

        # Продакшн-порядок: сначала сводим строки, потом строим матрицу.
        t.консолидировать_форм_строки(str(path))
        t.построить_матрицу_проверок(str(path))

        wb2 = load_workbook(str(path))
        # Две строки лога слились в одну (1 форма = 1 строка).
        assert wb2["Логи"].max_row == 2, вердикт
        ws2 = wb2["Москва"]
        got = {str(ws2.cell(r, 1).value): str(ws2.cell(r, 2).value)
               for r in range(2, ws2.max_row + 1)}
        assert got.get("Защита от XSS") == символ, (вердикт, got)
