...
# ── Кнопка «Отправить»: всегда-активная - норма (валидация по клику) ──
def test_кнопка_всегда_активная_норма():
    # Нажать на пустой форме можно, но заявка не уйдёт - подсветятся
    # незаполненные поля. Штатное поведение, не дефект.
...
# ── «Заявка ушла, подтверждение не поймали» → ⚠, а не ✗ ──
# Раньше такой случай (данные видны в теле POST, сервер ответил без ошибки, но
# явного «спасибо» не поймали) падал в «НЕТ ПОДТВЕРЖДЕНИЯ» = ✗ «форма не
# отправилась». Это ложное обвинение сайта: честный вердикт - ручная проверка.
def test_статус_заявка_ушла_подтверждение_не_поймали_предупреждение():
    assert _sym("Статус", "ПРОВЕРИТЬ (заявка ушла, подтверждение не поймали)") == "⚠"


def test_статус_заявка_ушла_распознаётся_как_строка_формы():
    # Иначе форма вообще не станет столбцом матрицы и молча пропадёт из отчёта.
    st = "проверить (заявка ушла, подтверждение не поймали)"
...
# ── Статус никогда не должен быть «значение не распознано» ──
# Регресс: у формы не записалась строка ОТПРАВКИ (проверка сорвалась), и при
# сведении «Статус» подтянулся от чужой проверки - «Проверить». В матрице это
# дало ⚠ «Значение "Проверить" не распознано»: человек не понимает ни что
# случилось, ни что делать.

def test_срыв_проверки_даёт_понятный_статус_а_не_нераспознано():
    сим, коммент = t._матрица_классифицировать(
        "Статус", "НЕ ПРОВЕРЕНО (нет строки отправки - проверка сорвалась)")
    assert сим == "⚠"
    assert коммент and "не распознано" not in коммент.lower()
    assert "не довёл проверку" in коммент


def test_статус_срыва_остаётся_столбцом_формы():
    # Иначе форма молча исчезнет из матрицы - худший исход.
    st = "не проверено (нет строки отправки - проверка сорвалась)"
    assert any(st.startswith(s) for s in t._МАТРИЦА_SUBMIT_ST)
