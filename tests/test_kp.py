...
def test_address_ne_rezhetsya_pochtovym_indeksom():
    """Почтовый индекс в начале адреса («198096, г. Санкт-Петербург, Гаванская
    улица, 4») НЕ должен обрезаться как телефон. Баг: стоп-маркер телефона
    «7/8 + 3 цифры» срабатывал на «8096» ВНУТРИ индекса 198096 и адрес
    схлопывался до «19» (кейс МПЭ/СПб, страница «Контакты»). Теперь 7/8 считается
    телефоном, только если перед ним НЕ цифра (реальное начало номера)."""
    from kp import _обрезать_хвост_адреса, _site_address_full, extract_site_contacts
    src = ('198096, г. Санкт-Петербург, Гаванская улица, 4 '
           'Телефон офиса продаж 7 (812) 986-43-98 Email spb@mepen.ru')
    assert _обрезать_хвост_адреса(src) == '198096, г. Санкт-Петербург, Гаванская улица, 4'
    # индекс В СЕРЕДИНЕ («город, индекс, улица», индекс с цифрой 8) - тоже не режем
    assert _обрезать_хвост_адреса('г. Санкт-Петербург, 198096, Гаванская улица, 4 '
                                  'Телефон 7 (812) 000-00-00') \
        == 'г. Санкт-Петербург, 198096, Гаванская улица, 4'
    # …но НАСТОЯЩИЙ телефон после адреса (перед 8 не цифра) по-прежнему обрезаем
    assert _обрезать_хвост_адреса('Гаванская улица, 4 8 495 000 00 00') \
        == 'Гаванская улица, 4'
    html = ('<main><h2>МетПромЭнерго в Санкт-Петербурге</h2>'
            '<p>Адрес: 198096, г. Санкт-Петербург, Гаванская улица, 4</p>'
            '<p>Телефон офиса продаж 7 (812) 986-43-98</p></main>')
    assert _site_address_full(html) == '198096, г. Санкт-Петербург, Гаванская улица, 4'


def test_check_variables_adres_s_indeksom_na_kontaktah():
    """Тот же кейс на уровне check_variables: адрес города у МПЭ лежит на странице
    «Контакты» с почтовым индексом. КП = «Гаванская улица, 4»; на сайте с индексом
    → Адрес ✓ (совпадает), а «Сайт» показывает чистый адрес, а не «19»."""
    import kp as kpmod
    row = kpmod.KPRow(domain='spb.mepen.ru', city='Санкт-Петербург',
                      phone_common='7 (812) 986-43-98', all_phones='8129864398',
                      email='spb@mepen.ru', address='Гаванская улица, 4',
                      country='Россия')
    main = '<header>spb@mepen.ru 7 (812) 986-43-98</header>'   # на главной адреса нет
    contacts = ('<main><h2>МетПромЭнерго в Санкт-Петербурге</h2>'
                '<p>Адрес: 198096, г. Санкт-Петербург, Гаванская улица, 4</p>'
                '<p>Телефон офиса продаж 7 (812) 986-43-98</p>'
                '<p>Email spb@mepen.ru</p></main>')
    r = kpmod.check_variables(main, 'spb.mepen.ru', contacts_html=contacts, row=row)
    af = next(f for f in r['fields'] if f['field'] == 'Адрес')
    assert af['status'] == 'ok', af
    assert af['found'] not in ('19', '–'), af          # не обрезано до «19»
    assert 'Гаванская' in af['found'], af
