...
def test_проблемная_форма_честные_крестики():
    ...
    assert симв['Подсказки полей'] == '✗'              # нет placeholder → ✗ (по требованию)
...
def test_согласие_рисует_js_прочерк_а_не_ложный_крестик():
    ...
    ссылка на политику есть. Вердикт по коду выносить нельзя: колонки согласия
    остаются прочерками, а не превращаются в ложные ✗.
    """
    html = '''
    ...
    print('✓ согласие рисует JS → прочерк, а не ложный ✗')
