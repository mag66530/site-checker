...
def test_старые_цели_убраны_из_прогона():
    """«Добавление товара в корзину» и «Открытие карточки товара» - старые цели.
    В Метрике у них в поле «идентификатор содержит» лежит ВЕСЬ вызов
    ym(99536626,'reachGoal','addtocart') (совпадения не будет никогда), а сайт
    таких идентификаторов не шлёт: добавление в корзину считают постраничные
    цели (*-v-korzinu). По решению 18.08.2026 убраны из прогона, но записаны в
    «исключены» - чтобы при следующей выгрузке из Метрики не вернулись молча."""
    c = _каталог()
    номера = {g['номер'] for g in c['цели']}
    assert '358243946' not in номера and '358244892' not in номера
    assert {x['номер'] for x in c['исключены']} == {'358243946', '358244892'}
    assert all(x['почему'] for x in c['исключены'])
    # И в «ожидаемых» плана их быть не должно - иначе прогон снова начнёт их
    # искать и показывать красным.
    план = G._план_страна('mpinew', 'https://new.metpromintex.by')
    assert 'addtocart' not in план['ожидаемые']
    assert 'cartopen' not in план['ожидаемые']
...
def test_план_проекта_свой_а_не_смушный():
    ...
    # Разделы шапки: у каждого свои цели, и их код лежит в отдельном чанке
    # Next.js - без захода на страницу цели читались бы как «нет в коде».
    for путь in ('/compares', '/favorites', '/selections'):
        assert 'https://new.metpromintex.by' + путь in urls, путь
...
def test_мусор_минификации_не_считается_привязкой():
    """Стенд на Next.js зовёт цель через переменную: ym(99536626,"reachGoal",e).
    Регэксп выхватывает оттуда «e» - и ОДНА такая фальшивая привязка ломала
    оценку прозрачности кода (не «>=3», но и не «привязок нет»), из-за чего
    правило «reachGoal в коде» не срабатывало ни для одной из 100 целей,
    реально лежащих в бандле."""
    assert G._чистые_привязки({'e', 'n', 'ym'}) == set()
    assert G._чистые_привязки({'e', 'Forma-Zadat-Vopros'}) == {'forma-zadat-vopros'}
    assert G._чистые_привязки(None) == set()
...
def test_формные_цели_не_исчезают_без_прогона_форм(monkeypatch):
    ...
    assert len(строки) == len(js_без_лида) == 103
