...
def test_активный_вердикт_блокирует_повтор_первая_прошла():
    # Первая отправка прошла, вторая не прошла (сервер не блокировал) - форма
    # сама блокирует повтор → «Блокирует повтор» (в матрице это ✓).
    статус, деталь = t.лимит_активно_вердикт([
        {"n": 1, "успех": True, "блок": False},
        {"n": 2, "успех": False, "блок": False},
    ])
    assert статус == "Блокирует повтор"
    assert "блокирует" in деталь.lower()
