...
def _план_для_домена(домен: str) -> dict:
    ...
    return {
        ...
        # На страны Хабаровск-цели не распространяются - помечаем «только Хабаровск».
        'нет_на_сайте': _ХАБАРОВСК_ТОЛЬКО,
    }
    ...
    for g in каталог.get('цели', []):
        ...
        if t == 'js':
            ...
            if _лид_цель(g):
                ...
                # значит цель ПРИВЯЗАНА и сработает при действии посетителя. Мы
                # сканируем весь код страниц разом и по этой привязке считаем цель
                # рабочей (goal-хит в прогоне мог не пойматься из-за таймингов beacon,
                # но привязка на месте - это надёжный сигнал, что цель настроена).
