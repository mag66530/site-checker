...
def _ссылка_с_url_частью(часть: str, код: str) -> bool:
    """На проверенных страницах ЕСТЬ ссылка (href), чей адрес содержит «часть».
    Значит клик по ней возможен, а Метрика такие клики/переходы (email, телефон,
    скачивание реквизитов/файла, внешняя ссылка, «Директ клик») считает САМА -
    своего reachGoal и перехода на страницу может не быть, обычным прогоном не
    поймать, но ссылка на сайте есть, и при клике посетителя цель зачтётся.
    `код` уже приведён к нижнему регистру."""
    часть = (часть or '').strip().lower()
    if not часть or not код:
        return False
    # mailto/tel: адрес идёт сразу за схемой (href="mailto:…"/"tel:…"); прочее -
    # фрагмент где-то внутри href (href="…/rekvizity-kompanii.pdf").
    try:
        return bool(re.search(r'href\s*=\s*["\'][^"\'>]*' + re.escape(часть), код))
    except re.error:
        return False
