...
def _классифицировать(pid: str, каталог: dict, прогон: dict) -> dict:
    ...
    # «Не найдена/найдено на сайте»: подтверждено (вручную или логикой плана),
    # что кнопки/формы под эту цель на сайте просто нет - напр. форма «Связаться
    # с менеджером» есть только на СНГ, а «Избранное» - только на хабаровском
    # поддомене. Проверять там нечего, в отчёте это лишь шум. По просьбе такие
    # строки не выводим и не учитываем в счётчиках (обе ветки клали их в «manual»).
    _скрытые_статусы = {'Не найдена на сайте', 'Не найдено на сайте'}
    _видимые = []
    for _r in _строки:
        if _r['статус'] in _скрытые_статусы:
            счёт['manual'] = max(0, счёт['manual'] - 1)
            continue
        _видимые.append(_r)
    _строки = _видимые
