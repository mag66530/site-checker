...
# ── Фильтры/санкции поисковых систем (п.1.19) ────────────────────────
# Санкции Яндекса (МПК, Баден-Баден, угрозы, навязчивая реклама) приходят
# в диагностику Вебмастера: FATAL - всегда санкция/угроза; прочие уровни
# отбираем по кодам-маркерам.
...
# Маркеры санкций в почтовых уведомлениях GSC (ручные меры / безопасность).
GSC_SANCTION_RE = re.compile(
    r'ручн\w+\s+мер|manual\s+action|security\s+issue|'
    r'проблем\w*\s+безопасност|взлом|hacked|вредонос|malware|deceptive|'
    r'фишинг|phishing', re.I)


def filter_sanctions(issues: list) -> list:
    """Санкционные сигналы из диагностики Вебмастера. issues - ServiceIssue
    (или dict из старого кеша). Возвращает список dict для отчёта."""
    out = []
    for i in issues or []:
        d = i.to_dict() if hasattr(i, 'to_dict') else dict(i)
        if d.get('service') != 'webmaster':
            continue
        sev = (d.get('severity') or '').lower()
        code = (d.get('code') or '').upper()
        if sev == 'fatal' or _SANCTION_CODE_RE.search(code):
            out.append(d)
    return out
