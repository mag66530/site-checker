...
def main() -> int:
    ...
    # По умолчанию прогоняем ВСЕ формы прямо в «Проверке целей» - цели форм
    # проверяются САМИ, без отдельного запуска «Проверки форм».
    ap.add_argument('--with-forms', action='store_true',
                    help='(устарело) полный прогон форм теперь идёт по умолчанию')
    ap.add_argument('--only-orders', action='store_true',
                    help='лёгкий режим: прогнать только сквозной заказ, а не все формы')
    ...
    # По умолчанию - ПОЛНЫЙ прогон форм прямо здесь: «Проверка целей» сама
    # отправляет все формы и подтверждает их цели, без отдельного запуска
    # «Проверки форм» и без «подтягивания» откуда-либо. --only-orders - лёгкий
    # режим (только сквозной заказ); --no-orders - вообще не трогать формы.
    if not a.no_orders:
        bases = []
        for p in projects:
            b = gt._базовый(p)
        ...
        for b in bases:
            if stop and stop():
                break
            _прогнать_формы(b, a.show_browser, only_orders=a.only_orders)
