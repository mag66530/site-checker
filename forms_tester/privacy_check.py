...
(Живочат/онлайн-консультант проверяет «Проверка целей» - здесь его не трогаем.)

Заходим на главную каждого проверенного города СВЕЖИМ контекстом браузера (без
cookie - значит «новый пользователь», плашка должна показаться) и пишем строки
...
def выполнить_проверку(города, excel_path: str = "log_forms.xlsx",
    ...
    log(f"🍪 Проверка 2.12 (cookie/политика) на {len(города)} главных …")
    есть_c = 0
