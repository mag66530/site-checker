...
def _stem_word(w: str) -> str:
    """Грубая основа русского слова для падежей. Прилагательные окончания
    «-ий/-ый/-ой/-ая/-ое…» режем на 2 буквы («Нижний»→«нижн», чтобы совпало с
    «Нижнем/Нижнего»); иначе отрезаем одну гласную/«ь»/«й» («Казань»→«казан»,
    «Москва»→«москв»)."""
    w = w.strip().lower()
    if len(w) > 5 and w[-2:] in (
            'ий', 'ый', 'ой', 'ей', 'ая', 'яя', 'ое', 'ее'):
        return w[:-2]
