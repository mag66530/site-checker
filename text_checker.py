...
Точная копия логики из Node.js версии:
  ...
  - #NAME#             - Битрикс-шаблоны мета (напр. #MIN_PRICE# в title -
                         «переменная минимальной цены», пункт чек-листа).
                         Учитывается ТОЛЬКО на товарных страницах с товаром
                         (фильтр filter_price_var_issues в http_checker)
...
BUILTIN_PATTERNS = {
    ...
    # Битрикс-шаблоны мета: #MIN_PRICE# и т.п. Имя с буквы, минимум 3
    # символа - чтобы не ловить якоря/хештеги/цвета (#fff - без закрытия).
    '#ПЕРЕМЕННАЯ#':    re.compile(r'#[a-zA-Zа-яА-Я_][a-zA-Zа-яА-Я0-9_]{2,40}#'),
