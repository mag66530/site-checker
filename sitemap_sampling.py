...
async def pick_sample_urls(groups: dict[str, list[str]], excluded: set,
                           ...
                           rng: Optional[random.Random] = None,
                           log=None) -> list[str]:
    """Полный проход: 1 файл на «вид» среди не исключённых → пул ссылок с
    каждого выбранного файла. Цель по числу карт - не фиксированное число, а
    ровно число видов, найденных НА ЭТОМ САЙТЕ (excluded=∅ в режиме «Рандом»,
    заполненный набор - в режиме «Вручную»)."""
