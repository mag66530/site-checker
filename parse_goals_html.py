...
# Идентификатор цели может быть МНОГОСЛОВНЫМ: часть сайтов зовёт reachGoal с
# пробелом внутри строки, напр. ym(id,'reachGoal','button send_express_request').
# Тогда id - это вся строка до запятой (разделителя ids), а не первое слово.
# Для обычных однословных ids поведение прежнее (захватывается один токен).
_ID_TOKEN = re.compile(
    r'идентификатор(?:\s+цели)?(?:\s+содержит)?\s*:\s*([\w\-.]+(?:\s+[\w\-.]+)*)')
_AUTO_PHRASES = (
    ...
    'соц. сет', 'соц.сет', 'все соц', 'оформлени заказа', 'оформления заказа',
    # Цели, собранные КОНСТРУКТОРОМ Метрики (без reachGoal в коде сайта):
    # клик по кнопке-селектору, поиск по сайту, поведенческие. Метрика считает
    # их сама на своей стороне - проверить извне нельзя, и «составными» они не
    # являются. Раньше всё это падало в «не распознано» → «Составная».
    'клик по кнопке', 'поиск по сайту', 'просмотр ', 'продолжительность визита',
    'глубина просмотра', 'посещение страниц',
)
# Цели на ОТПРАВКУ формы, заведённые конструктором («форма», «все формы»).
# Их проверяет «Проверка форм», а не прогон целей.
_FORM_PHRASES = ('форма', 'формы', 'все формы')
...
def _классифицировать(g: dict) -> dict:
    ...
    if 'идентификатор' in low:
        ...
        # В Метрике условие иногда записывают ЦЕЛИКОМ вызовом:
        # «идентификатор содержит: ym(123,'reachGoal','addtocart')». Тогда
        # первым токеном идёт «ym», и проверка искала бы в коде именно его -
        # а «ym» есть на любом сайте со счётчиком, то есть цель всегда
        # выглядела бы настроенной. Берём настоящий идентификатор из вызова.
        _из_вызова = re.findall(r"reachGoal'?\s*,\s*'([\w\-.]+)'", c)
        if _из_вызова:
            ids = _из_вызова
    ...
    if any(p in low for p in _AUTO_PHRASES) or low == 'составная цель' or not c:
        g['тип'] = 'auto'
        return g
    # Цель на отправку формы, заведённая конструктором («форма»/«все формы»):
    # идентификатора в коде у неё нет, срабатывает при реальной отправке -
    # её и подтверждает «Проверка форм». Помечаем как форменную (тип js без
    # идентификаторов + флаг), чтобы отчёт писал «проверяется формами», а не
    # бесполезное «Составная».
    if low in _FORM_PHRASES or low.startswith('форма'):
        g['тип'] = 'js'
        g['форма'] = True
        return g
