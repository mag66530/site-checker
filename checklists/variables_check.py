...
with st.expander('📄 Источник данных (Карта присутствия)', expanded=False):
    ...
    if _has_url:
        ...
        if st.button('↻ Обновить КП из Google сейчас', key=f'kp_refresh_{pid_key}',
                     help='После правок в таблице подожди ~2 минуты: Google кэширует '
                          'экспорт и сразу после изменения отдаёт старые данные. '
                          'Через пару минут нажми — подтянется свежее.'):
