...
«Главные переменные» - страница тайлит лог и показывает прогресс.
"""
import json
import os
import subprocess
...
def _render_recs(data: dict):
    ...
    for rec in recs:
        ...
        head = f'**{title}**  ·  на {pages} стр.' + (f'  ·  {savings}' if savings else '')
        with st.expander(head, expanded=False):
            items = rec.get('items', [])
            if items:
                st.caption('Что и где конкретно:')
                for it in items:
                    info = f' — {it["info"]}' if it.get('info') else ''
                    st.markdown(f'- `{escape(it.get("url",""))}`{escape(info)}')
...
st.caption('Google PageSpeed / Lighthouse: оценка скорости (desktop + mobile) по '
           ...
           '(что и где чинить). Отчёт - Excel. История хранится локально.')
...
with st.expander('🗂 История прогонов (перенос между машинами)'):
    ...
    if PH is not None:
        ...
        with cc2:
            up = st.file_uploader('⬆ Загрузить историю (CSV)', type=['csv'], key=f'ps_up_{pid}')
            _replace = st.checkbox('Заменить целиком (иначе добавить недостающее)',
                                   key=f'ps_rep_{pid}')
            if up is not None and st.button('Применить загрузку', key=f'ps_apply_{pid}'):
                added = PH.import_csv(pid, up.getvalue(),
                                      mode='replace' if _replace else 'merge')
                st.success(f'Готово: добавлено строк — {added}.')
                st.rerun()
