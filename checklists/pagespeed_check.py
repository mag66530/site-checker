...
_tpl.render_panel(
    ...
    help_text='Шаблон запоминает охват, число страниц каждого типа, включение '
              'товаров и период сравнения (ключ PageSpeed API не сохраняется). '
              'Хранится на сервере проекта **до перезапуска приложения** - после '
              'может сброситься.')
...
compare = st.selectbox('Сравнивать с периодом',
                       ['prev', 'week', 'month'], index=0, key=f'ps_compare_{pid}',
...
with c1:
    if st.button('▶ Запустить проверку скорости', use_container_width=True,
                 type='primary', disabled=alive or _no_urls):
...
with st.expander('📊 Все прогоны – сравнение со всеми прошлыми снятиями'):
    ...
    if not runs_agg:
        ...
        # Скачать сравнение всех прогонов одним Excel (лист «Все прогоны» +
        # лист «По типам»).
        try:
            import pagespeed_report as PR
            _allx = PR.all_runs_to_bytes(project_name=PROJECTS.get(pid, pid),
                                         runs_agg=runs_agg)
            st.download_button(
                '⬇ Скачать все прогоны (Excel)', data=_allx,
                file_name=f'{pid.upper()}-все-прогоны.xlsx',
                mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                use_container_width=True, key=f'ps_allruns_dl_{pid}')
        except Exception as e:  # noqa: BLE001
            st.caption(f'Не удалось собрать файл для скачивания: {e}')
...
# В КОНЦЕ страницы: настройки охвата уже отрисованы, значения в session_state.
# Ключ PageSpeed API (ps_key_input_*) сюда НЕ входит - пароли не сохраняем.
_tpl.commit_pending('pagespeed', pid, [
    f'ps_scope_{pid}', f'ps_pertype_{pid}', f'ps_wantprod_{pid}',
