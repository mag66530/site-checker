    ...
    with st.container(border=True):
        ...
        _ck_adm = st.checkbox(
            ...
            help='Браузер заходит в админку Bitrix и проверяет, что разделы '
                 'настройки открываются и работают: мастер поддоменов, разделы '
                 'каталога, товарная подсистема (HL-блок «Ассортимент»), '
                 'структура сайта и редактор файлов тех.страниц. Только '
                 'открытие/рендер, ничего не меняется. Отдельный лист '
                 '«Настройки в админке».')
        _ck_crud = st.checkbox(
            ...
            help='CRUD-функции. Поддомены: создание (симуляция-dry-run, на '
                 ...
                 'загрузка (импорт). В отчёт идёт аудит «было → стало».')
        if _ck_crud:
            st.checkbox('↳ Тест-выполнение с откатом (реально писать в БД)',
                        key='c30_adm_execute',
                        help='ВКЛ (рекомендуется): реально создаёт-правит-'
                             'скрывает-удаляет временный СКРЫТЫЙ раздел '
                             '«[ТЕСТ ЧЕКЕРА]» (без товаров, удаляется сразу) и '
                             'прогоняет симуляцию поддомена - аудит «было → '
                             'стало». ВЫКЛ: только наличие CRUD-функций (формы/'
                             'кнопки), ничего не пишется. Выключи, если любых '
                             'записей на проде не хочешь.')

        if _ck_adm or _ck_crud:
            ...
            if _ck_crud:
                st.caption('CRUD категорий выполняется на временном скрытом '
                           'разделе и откатывается; поддомены реально не '
                           'создаются (симуляция) и не удаляются (наличие '
                           'функции). Обкатано на тестовом контуре.')
