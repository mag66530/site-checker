    ...
    with st.container(border=True):
        ...
        with _sec_teh.expander(f'⚙️  Техничка – доступность, индексация, дубли, метаданные, 404, скорость  `{_sec_n(_SEC_TEH)}/{len(_SEC_TEH)}`', expanded=True):
            ...
            st.checkbox('Сжатие (Gzip/Brotli) и кеширование статики',
                        ...
                        help='Проверяем, что CSS/JS сайта отдаются сжатыми (Gzip/'
                             ...
                             'страница грузится тяжелее и каждый раз заново. Как '
                             'проверить руками: F12 → вкладка Network → обновить '
        ...
        with _sec_con.expander(f'🖼️  Контент – изображения  `{_sec_n(_SEC_CON)}/{len(_SEC_CON)}`', expanded=True):
            st.checkbox('Изображения (alt, webp/avif, вес, имена файлов)',
                        ...
                        help='Alt у всех <img> (баг - и пустой alt="", и полное '
                             'отсутствие атрибута). Современные форматы webp/avif (иначе '
        ...
        with _sec_kp.expander(f'🗺️  Данные по городам  `{_sec_n(_SEC_KP)}/{len(_SEC_KP)}`', expanded=True):
            ...
            st.checkbox('СНГ-домены: нет упоминаний РФ, СНГ и чужих стран',
                        ...
                        help='На сайте страны СНГ (домены .kz/.by/.uz/…) в текстах, '
                             ...
                             'Как проверить руками: откройте сайт нужной страны и '
        ...
        with _sec_ana.expander(f'📊  Аналитика – ссылочный профиль, аномалии, трафик, траст  `{_sec_n(_SEC_ANA)}/{len(_SEC_ANA)}`', expanded=True):
            ...
            st.checkbox('Отзывы: приоритет докупки (Я.Бизнес + 2ГИС)',
                        ...
                        help='Единая проверка отзывов (таблица внизу листа «Я.Бизнес '
                             'и GMB», включайте вместе с проверкой Я.Бизнеса). По '
                             'конфигу catalogs/reviews-<проект>.csv (city, yandex_url, '
                             '2gis_url) тянет живьём рейтинг и число отзывов каждого '
                             'филиала из Яндекс.Бизнеса и 2ГИС, строит приоритет '
                             'докупки: сначала филиалы с рейтингом < 4.7, затем города '
                             'от миллионников к меньшим; докупаем по 2 отзыва (3 при '
                             'низком рейтинге). Пока настроен только smu (для mpe/imp '
                             'нужен свой CSV).')
            st.checkbox('Аномалии: мусорные запросы (ГСК) + переходы с мусорных сайтов (Метрика)',
                        key='c30_check_anomalies',
                        help='Ищет негативное SEO. ГСК (Search Analytics по сервисному '
                             'аккаунту): всплеск показов по мусорным/иноязычным запросам '
                             'за 14 дней vs предыдущие 14. Метрика: переходы с '
                             'доменов-рефереров, похожих на спам (мусорные TLD / '
                             'gambling / adult), за 30 дней vs предыдущие 30, + всплеск. '
