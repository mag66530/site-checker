    ...
    with st.container(border=True):
        ...
        st.checkbox('1.16  Валидация W3C (HTML/CSS) + скорость ресурсов',
                    ...
                    help='По выборке страниц (главная/категория/товар): HTML '
                         ...
                         '⚠ ВАЖНО: W3C - бесплатный сервис с ЛИМИТОМ запросов. '
                         'Для РЕДКОГО использования (раз в день/несколько дней). '
                         'Если включать часто - W3C временно заблокирует (HTTP '
                         '403), и валидность HTML/CSS не проверится (в отчёте '
                         'будет «не проверено - лимит, повторить позже»). '
                         'Скорость ресурсов при этом всё равно измеряется.')
