...
if pid:
    ...
    with st.container(border=True):
        ...
        with _cb_col2:
            ...
            st.caption('⚠ 1.16 использует бесплатные сервисы W3C с ЛИМИТОМ '
                       'запросов. Включай РЕДКО (раз в день / несколько дней): '
                       'при частом прогоне W3C временно заблокирует (HTTP 403) '
                       'и валидность HTML/CSS не проверится - в отчёте будет '
                       '«не проверено, повторить позже». Скорость ресурсов '
                       'измеряется в любом случае.')
