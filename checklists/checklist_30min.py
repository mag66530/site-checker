...
if pid:
    ...
    with st.container(border=True):
        ...
        with _sec_teh.expander(f'⚙️  Техничка – индексация, дубли, метаданные, 404, скорость  `{_sec_n(_SEC_TEH)}/{len(_SEC_TEH)}`', expanded=True):
            ...
            st.checkbox('Проверять страницы в индексе: 404 и служебные адреса '
                        '(Яндекс.Вебмастер + Google)',
                        key='c30_check_index_404',
                        help='**Ищем в индексе страницы, которые отдают '
                             '404/410/5xx, и служебные адреса, которым в '
                             'поиске не место (корзина, оформление заказа, '
                             'поиск по сайту, сравнение, личный кабинет, '
                             'авторизация, админка, AJAX-обработчики).**\n\n'
                             '- служебный адрес попадает в отчёт, только если '
                             'он СЕЙЧАС отвечает 200 и на нём нет noindex - '
                             'иначе поисковик просто помнит старое\n'
                             ...
                             'Источники объединяются; находки - на листе '
                             '«Проблемы», разделы «404 в индексе» и '
                             '«Тех. страницы в индексе».')
        ...
        with _sec_ver.expander(f'🎨  Вёрстка – адаптивность, навигация, фильтрация  `{_sec_n(_SEC_VER)}/{len(_SEC_VER)}`', expanded=True):
            st.checkbox('Вёрстка, адаптивность и навигация (viewport, стили, меню)',
                        ...
                        help='**Проверяем адаптивность, вёрстку и навигацию сайта.**\n\n'
                             ...
                             '- теги-артефакты форматирования (<strong>, <b>, <em>, '
                             '<i> с текстом) в подзаголовках h2-h6, шапке/меню и '
                             'подвале - там жирность и курсив задаются стилями. '
                             'Иконочные <i> (пустые или с классом icon/fa-) не '
                             'считаются\n\n'
