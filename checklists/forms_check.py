...
with st.expander('📁 Шаблоны прогона (сохранить/выбрать набор городов и форм)',
    ...
    # Сообщение об успешном сохранении показываем ПОСЛЕ rerun - чтобы новый шаблон
    # уже был в списке выше. Флаг ставит кнопка «Сохранить» внизу страницы.
    _saved = st.session_state.pop(f'tpl_saved_{pid_key}', '')
    if _saved:
        st.success(f'Шаблон «{_saved}» сохранён и добавлен в список выше.')
...
with st.expander('💾 Сохранить текущий выбор как шаблон', expanded=False):
    ...
    if _sv2.button('Сохранить', use_container_width=True,
        ...
        # Перерисовываем страницу - чтобы новый шаблон сразу появился в списке
        # вверху («Шаблоны прогона»), без ручного обновления. Сообщение об успехе
        # покажется там же после перерисовки.
