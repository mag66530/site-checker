...
# После сохранения шаблона этот раздел раскрываем автоматически - чтобы новый
# шаблон сразу был на виду. Флаг снимаем сразу (раскрыто на один заход).
_tpl_open = bool(st.session_state.pop(f'tpl_open_{pid_key}', False))
with st.expander('📁 Шаблоны прогона (сохранить/загрузить настройки страницы)',
                 expanded=_tpl_open):
    _just_saved = st.session_state.pop(f'tpl_saved_{pid_key}', '')
    if _just_saved:
        st.success(f'Шаблон «{_just_saved}» сохранён - теперь он в списке ниже.')
    _tpls = _tpl_load_all()
    ...
    _tpl_new = _sv1.text_input(
        'Сохранить текущие настройки как шаблон', key=f'tpl_name_{pid_key}',
...
st.caption('ℹ️ Наличие капчи и honeypot-поля на форме проверяется ВСЕГДА, без '
           'галочек и без единого лишнего запроса - смотри колонки «CSRF-защита», '
           '«Данные дошли до сервера» и «Защита от спама (пассивно)» в отчёте.')
