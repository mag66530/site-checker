...
st.caption('Формы проверяются автоматически по коду (быстро, requests), где это '
           'возможно; где форму по коду не видно (её рисует/шлёт JS) или включены '
           'браузерные пробы ниже - через браузер. Отдельно ничего включать не '
           'нужно.')
...
st.caption('Только для форм с полем загрузки файлов. По очереди грузит '
           'БЕЗВРЕДНЫЙ файл 32 типов (13 опасных: .php/.svg/.exe/.js/.bat/.sh/'
           '.py/.jar/.dll/.msi/.html/.htm/.phtml, обычные: pdf/doc/docx/xlsx/'
           'zip/jpg/png…) и отправляет форму - проверяет, что примет сервер. '
           'Отдельно грузит РАЗРЕШЁННЫЙ тип больше 20 КБ - проверяет лимит '
           'размера. Правило: принимать только PDF/DOC/DOCX до 20 КБ; всё '
           'остальное (посторонний формат ИЛИ больше 20 КБ) должно '
           'отклоняться. Колонка «Типы файлов формы»: «✗» = принят опасный/'
           'посторонний тип или файл больше 20 КБ; «корректно» = правило '
           'соблюдено. ВНИМАНИЕ: каждый принятый тип = отдельная тест-заявка '
           'в админке боевого сайта.')
...
st.caption('Проверяет пункт «нельзя отправить неверные данные через DevTools»: '
           ...
           'обязательное поле / БЕЗ ИМЕНИ / БЕЗ ТЕЛЕФОНА / e-mail без «@» / '
           'слишком длинный текст). В отчёте пишет КОНКРЕТНО, что прошло '
           '(напр. «ПРОШЛА без телефона (поле «phone»)»). Колонка «Серверная '
           'валидация»: Защищена / УЯЗВИМА / Проверить. Формы оформления заказа '
           'пропускаются. ВНИМАНИЕ: если сервер примет невалидные данные - это '
           'до 5 доп. тест-заявок на форму в админке боевого сайта (остальные '
           'поля остаются с тест-данными, поэтому заявку видно в админке) - '
           'после прогона удалите их вручную.')
