...
# После сохранения шаблона этот раздел раскрываем автоматически и ЧИСТИМ поле
# имени - чтобы было ВИДНО, что сохранение сработало (раньше поле оставалось
# заполненным, подтверждение мигало один раз - и казалось, что «ничего не
# поменялось»). Флаги снимаем сразу (на один заход).
_tpl_open = bool(st.session_state.pop(f'tpl_open_{pid_key}', False))
if st.session_state.pop(f'tpl_clear_name_{pid_key}', False):
    st.session_state[f'tpl_name_{pid_key}'] = ''
with st.expander('📁 Проектные шаблоны (сохранить/загрузить настройки страницы)',
    ...
    if _just_saved:
        st.toast(f'Шаблон «{_just_saved}» сохранён', icon='✅')
        st.success(f'✓ Готово! Шаблон «{_just_saved}» сохранён и появился в списке '
                   '«Загрузить шаблон» ниже - его можно применить одной кнопкой.')
    st.caption('Как это работает: **1)** настройте страницу как нужно (города, '
               'формы, галочки) → **2)** впишите название внизу → **3)** нажмите '
               '«💾 Сохранить». Потом эти же настройки вернёте кнопкой «Загрузить».')
    _tpls = _tpl_load_all()
    # Загрузить сохранённый шаблон (проставит все настройки страницы разом).
    if _tpls:
    ...
    # Сохранить текущие настройки как новый шаблон. Кнопка ставит флаг, а сам
    # шаблон собирается ниже по странице (когда отрисованы все настройки).
    ...
    if _sv2.button('💾 Сохранить', use_container_width=True, type='primary',
        ...
        st.session_state[f'tpl_pending_{pid_key}'] = _tpl_new.strip()
        st.rerun()
...
# ── Отложенное сохранение шаблона (кнопка «Сохранить» - в блоке вверху) ──
# Кнопка ставит флаг tpl_pending и делает rerun; сам шаблон собираем ЗДЕСЬ, когда
# уже отрисованы города/формы/настройки (их значения лежат в session_state).
...
if _tpl_pending:
    ...
    st.session_state[f'tpl_clear_name_{pid_key}'] = True   # очистить поле имени -
    #                                    видно, что сохранилось, и не дублируется
    st.rerun()
