...
PROJECTS = {
    ...
    # У МТТ ДВА интерфейса, поэтому два проекта: старый - на всех доменах,
    # новый - только Армения (другая вёрстка, другие формы).
    'mtt': {'name': 'МТТ - Меттранстерминал', 'domain': 'met-trans.ru'},
    'mtt_am': {'name': 'МТТ - Меттранстерминал (Армения)', 'domain': 'met-trans.am'},
}
...
def _tpl_reset():
    """Сброс страницы к стандартным настройкам: убираем ключи виджетов и флаги
    инициализации из session_state - на следующем прогоне проставятся дефолты
    (режим «Основные домены», все домены отмечены, галочки запуска по умолчанию)."""
    for _kk in [f'fc_mode_{pid_key}', f'fc_forms_mode_{pid_key}',
                f'fc_init_osn_{pid_key}', f'ff_init_{pid_key}',
...
with st.expander('📁 Проектные шаблоны (сохранить/загрузить настройки страницы)',
    ...
    if _tpls:
        ...
        # настройкам» (ловим переход «был выбор → стало пусто»).
        _prevk = f'tpl_pick_prev_{pid_key}'
        if st.session_state.get(_prevk) and not _pick:
            st.session_state[_prevk] = None
            _tpl_reset()
            st.toast('Настройки страницы сброшены к стандартным', icon='🔄')
            st.rerun()
        st.session_state[_prevk] = _pick
        ...
        if _tc3.button('Удалить', use_container_width=True,
            ...
            _tpl_save_all(_tpls)
            st.rerun()
        st.caption('💡 Крестик (×) в поле «Загрузить шаблон» вернёт страницу к '
                   'стандартным настройкам (все домены, галочки по умолчанию).')
    else:
...
# Разделитель перед блоком письма - только когда выше показана «Проверка
# админки» (иначе после списка форм уже есть divider, двойной не рисуем).
if _project_has_admin(pid_key):
    st.divider()
...
# Явный разделитель перед секцией запуска: галочки проверок + прокси + кнопка
# «Запустить проверку» отделены от «Проверки письма о заказе» линией, а не
# только отступом - секции не сливаются.
st.divider()
