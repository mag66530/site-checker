...
st.session_state.setdefault(f'fc_fast_{pid_key}', False)
fast_requests = st.checkbox('⚡ Быстрая проверка по коду (requests, без браузера)',
                            key=f'fc_fast_{pid_key}')
st.caption('Быстро: обычные формы проверяются запросом по коду (HTML + POST), без '
           'запуска браузера - заявка при этом всё равно отправляется. НО '
           'браузерные проверки в этом режиме НЕ идут: защита от XSS, двойная '
           'отправка, серверная валидация, лимит запросов, стили, модалки, Enter. '
           'Многошаговые формы оформления заказа всё равно проходят через браузер '
           '(по коду их не пройти). Годится для быстрой проверки «форма на месте и '
           'заявка уходит»; для полной проверки безопасности - снимите галочку.')
if fast_requests:
    st.caption('⚡ Режим «по коду» включён - браузерные пробы ниже отключены.')
...
file_probe = st.checkbox('Проба серверной фильтрации загрузки файлов',
                         key=f'fc_fileprobe_{pid_key}', disabled=fast_requests)
...
srv_val_probe = st.checkbox('Проба серверной валидации', key=f'fc_srvval_{pid_key}',
                            disabled=fast_requests)
st.caption('Проверяет пункт «нельзя отправить неверные данные через DevTools»: '
           'после обычной отправки формы ЕЩЁ РАЗ пробует отправить её, но '
           'предварительно через JS снимает клиентские ограничения '
           '(обязательность/формат/длину) - как если бы кто-то поправил форму '
           'в DevTools браузера - и подставляет заведомо неверные данные (пустое '
           'обязательное поле / e-mail без «@» / слишком длинный текст). '
           'Колонка «Серверная валидация»: Защищена / УЯЗВИМА / Проверить. '
           'Формы оформления заказа пропускаются. ВНИМАНИЕ: если сервер примет '
           'невалидные данные - это до 3 доп. тест-заявок на форму в админке '
           'боевого сайта (там, где это возможно, с меткой «ТЕСТ-ВАЛИДАЦИЯ» в '
           'тексте) - после прогона удалите их вручную.')
st.caption('ℹ️ Наличие капчи и honeypot-поля на форме проверяется ВСЕГДА, без '
           'галочек и без единого лишнего запроса - смотри новые колонки '
           '«CSRF-защита», «Данные дошли до сервера» и «Защита от спама '
           '(пассивно)» в отчёте.')
st.session_state.setdefault(f'fc_ratelimit_{pid_key}', False)
rate_limit_probe = st.checkbox('🧪 Активная проверка лимита запросов',
                               key=f'fc_ratelimit_{pid_key}', disabled=fast_requests)
st.caption(
    'Что это: отправляет по этой форме **3 тестовые заявки подряд** за '
    'несколько секунд - чтобы проверить, остановит ли сайт после нескольких '
    'быстрых попыток (это и есть «защита от спама/ботов»), или пропустит все '
    'подряд, как настоящего бота-спамера.')
st.caption(
    'Зачем это нужно, если наличие капчи и так видно всегда (см. выше): та '
    'проверка только смотрит, ЕСТЬ ли на форме капча или похожий значок '
    'защиты - но не проверяет, что он на самом деле СРАБАТЫВАЕТ при быстрых '
    'повторах. Эта галочка отвечает на вопрос по-настоящему - как если бы '
    'кто-то попробовал быстро закинуть несколько заявок вручную.')
st.caption(
    '⚠️ Важно: если защиты нет, все 3 попытки попадут в админку клиента как '
    'настоящие заявки (их придётся удалить вручную). Не включайте галочку '
    'повторно без необходимости - каждый запуск это ещё 3 заявки на боевом '
    'сайте. Число попыток жёстко ограничено (3) - программа сама «пробовать '
    'ещё» не будет. Формы оформления заказа галочка не трогает.')
