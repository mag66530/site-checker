...
# ── Формы внутри целей ───────────────────────────────────────────────
# Формы прогоняются ВСЕГДА (проверять цели без целей отправки форм смысла нет).
# Отдельной галочки нет - это часть проверки целей.
st.caption('📨 Перед проверкой целей автоматически прогоняются формы (реальные '
           'тестовые заявки на почту по основному сайту, Москва) - чтобы цели '
           'отправки форм попали в отчёт как «Сработала (формы)». Это часть '
           'проверки, отдельно включать не нужно.')
...
with c1:
    if st.button('▶ Запустить проверку целей', use_container_width=True,
        ...
        # Формы прогоняем всегда (галочку убрали) - цели отправки форм это часть
        # проверки целей.
        args = [PY, 'goals_run.py', '--projects', ','.join(_selected), '--with-forms']
