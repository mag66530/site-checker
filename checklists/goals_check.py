...
СТРАНЫ = {
    ...
    # STB - счётчик на каждую страну (номера вытащены из кода самих сайтов).
    # Города РФ сидят на поддоменах с общим счётчиком, поэтому проверяем по
    # одному сайту на страну. У России 15 своих js-целей на события форм, у
    # зарубежных - ТОЛЬКО автоцели Метрики (своих событий не заведено): там
    # проверять по сути нечего, пока клиент не настроит цели.
    'stb': [
        ('stb',    'Россия · steelborg.ru'),
        ('stb-kz', 'Казахстан · steelborg.kz'),
        ('stb-rb', 'Беларусь · steelborg.by'),
        ('stb-uz', 'Узбекистан · steelborg.uz'),
        ('stb-kg', 'Кыргызстан · steelborg.kg'),
        ('stb-az', 'Азербайджан · steelborg.az'),
    ],
    'mtt': [
...
# ── Проектные шаблоны ────────────────────────────────────────────────
# Галочки сайтов лежат в ключах вида gc_cb_<проект>_<сайт> (см. _ck ниже).
# save_keys читает session_state в момент клика «Сохранить» - к тому времени
# галочки уже отрисованы в прошлом прогоне и лежат в состоянии.
_tpl.render_panel(
    'goals', _base,
...
if log_txt:
    ...
    # Во время прогона лог раскрыт (живое наблюдение), по завершении -
    # свёрнут, но открывается кликом. Кнопка «Скачать лог» остаётся ниже.
...
# (render_panel(save_keys=...)) - не зависит от авто-rerun прогона внизу.
