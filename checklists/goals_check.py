...
# ── Формы внутри целей ───────────────────────────────────────────────
# Проверка целей САМА формы НЕ отправляет (не спамим реальными заявками). Цели
# отправки форм становятся зелёными по «reachGoal в коде», а результат заказов
# («Сделан заказ»/«Оформил заказ») подтягивается из ПОСЛЕДНЕГО прогона «Проверки
# форм» - запустите её отдельно, когда нужно подтвердить заказы end-to-end.
st.caption('📨 Проверка целей НЕ отправляет формы - цели отправки форм зелёные по '
           '«reachGoal в коде». Цели-«заказы» подтягиваются из последнего прогона '
           '«Проверки форм» (запустите её отдельно, если нужно подтвердить заказ).')
...
with c1:
    if st.button('▶ Запустить проверку целей', use_container_width=True,
        ...
        # Формы НЕ шлём: цели форм зелёные по «reachGoal в коде», а заказы берём
        # из последнего прогона «Проверки форм» (её результат подтягивается сам).
