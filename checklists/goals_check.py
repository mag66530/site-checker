...
# Дефолт (один раз на проект): все с доменом включены, без домена - выключены.
if not st.session_state.get(f'gc_init_{_base}'):
    for p, _ in _варианты:
        st.session_state[_ck(p)] = _домен_ок(p)
    st.session_state[f'gc_init_{_base}'] = True
