...
if len(_варианты) > 1:
    pid = st.selectbox('Страна / сайт', [v[0] for v in _варианты],
                       format_func=lambda k: dict(_варианты)[k])
else:
    pid = _варианты[0][0]
...
_домен = (каталог.get('домен') or '').strip()
st.markdown(f"**{каталог.get('проект','')}** · счётчик `{каталог.get('счётчик','')}` · "
            f"целей в каталоге: **{len(каталог.get('цели', []))}**"
            + (f" · сайт `{_домен}`" if _домен else ''))

# Базовые проекты знают свои страницы (ACTIONS); странам нужен домен из каталога.
_домен_ок = bool(_домен) or pid in ('smu', 'imp', 'mpe')
if not _домен_ок:
    st.warning('Для этой страны ещё не задан адрес сайта (домен) - проверку целей '
               'не запустить. Пришлите домен, и я его пропишу в каталог.')
...
with c1:
    if st.button('▶ Запустить проверку целей', use_container_width=True,
                 disabled=_alive or not _bok or not _домен_ок):
