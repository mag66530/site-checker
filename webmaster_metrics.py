...
Что считаем аномалией:
  1. Обход (по истории Яндекса, эталон хранить не нужно):
     • всплеск 5xx (сервер отдаёт ошибки роботу) - ×2 и не меньше floor;
     • всплеск 4xx (массовые 404 - страницы удалили/выпали) - ×2 и floor.
     (Просадку 2xx НЕ считаем: число обойдённых за период страниц само
     сильно колеблется - это шум. Реальную потерю ловим по searchable ниже.)
  2. Проблемы сайта (текущий снимок): только FATAL - красный флаг сам по себе
     (эталон не нужен). CRITICAL - шум (есть почти у всех), не тащим.
...
def analyze_crawl(indicators):
    """Аномалии обхода из /indexing/history: ВСПЛЕСК ошибок 4xx/5xx, которые
    робот получил при обходе. Возвращает [{metric, before, after, delta_pct,
    severity, text}].

    Просадку 2xx НЕ считаем: HTTP_2XX здесь - сколько страниц робот обошёл ЗА
    ПЕРИОД, а это число само сильно колеблется (сегодня обошёл 50, завтра 10) -
    получается шум, а не «страницы выпали». Реальную потерю страниц ловим по
    searchable_pages_count из /summary (analyze_summary)."""
    ind = indicators or {}
    out = []
    # 4xx / 5xx - всплеск относительно недавнего фона; floor гасит мелочь
    # (несколько 404 - норма, а не аномалия).
    for code, label, sev, floor in (
            ('HTTP_4XX', 'Обход: ошибки 404 (4xx)', 'critical', 15),
            ('HTTP_5XX', 'Обход: ошибки сервера (5xx)', 'fatal', 10)):
