...
банк:
  ...
  • CSP: отсутствие НЕ ругаем (часто нет и это норма); если есть, но с
    unsafe-inline И unsafe-eval сразу - защита от XSS выключена =
    предупреждение.
...
def check_security_headers(headers: Optional[dict], url: str = '') -> Optional[dict]:
    ...
    # ── CSP: отсутствие не ругаем; ловим заведомо бесполезную настройку ──
    if csp:
        present.append('CSP')
        low = csp.lower()
