...
def _fetch_one(dom, proxy_parts, ua=None, path='/'):
    ...
    def _get(host, path, depth=0):
        ...
        try:
            ...
            # Пробиваем ЛЮБОЙ кэш (CDN/Cloudflare): свежее соединение без кук +
            # уникальная метка в URL + no-store. По сути «инкогнито, кэш сброшен».
            _p = path or '/'
            _bust = f"_nc={int(time.time() * 1000)}{depth}"
            _p = _p + ('&' if '?' in _p else '?') + _bust
            conn.request('GET', _p, headers={
...
def _варианты_города(city: str) -> list:
    """Названия города, любое из которых на сайте считается «своим».

    В КП встречается двойное имя в скобках - «Нур-Султан (Астана)». Сайт пишет
    ОДНО из них (ns.mtt.kz - «Астана»), а проверка искала всю строку целиком и
    честно писала «город из КП на сайте не найден». Возвращаем оба варианта: и
    то, что до скобки, и то, что внутри. ЧИСТАЯ функция - есть юнит-тест."""
    s = (city or "").strip()
    if not s:
        return []
    m = re.match(r"^(.+?)\s*\(([^)]+)\)\s*$", s)
    if not m:
        return [s]
    return [x for x in (m.group(1).strip(), m.group(2).strip()) if x]
...
def _own_city_regex(city):
    ...
    Двойное имя из КП («Нур-Султан (Астана)») разбираем на варианты и ищем
    ЛЮБОЙ из них - см. _варианты_города."""
    import re as _re
    _вар = _варианты_города(city)
    if len(_вар) > 1:
        _части = [rx.pattern for rx in (_own_city_regex(v) for v in _вар) if rx]
        return _re.compile("|".join(f"(?:{p})" for p in _части), _re.I) \
            if _части else None
...
def _регион_статусы(html, host, ctx):
    ...
    # В КП города нет, но на сайте он ВЫВЕДЕН → расхождение ✗ (на сайте есть, в
    # КП нет; жёлтых значков в отчёте больше не используем).
    if not свой:
        _site_city = _город_на_сайте(html)
        if _site_city:
            город.update(found=_site_city, status="bug",
                         note="город на сайте не совпадает с КП")
...
def _только_почта_для_перевода(city: str, fields: list) -> list:
    ...
    → «Азербайджан (перевод)»). Это реальный сайт со своими контактами - телефоны/
    почту/адрес/мессенджеры сверяем как обычно. Гасим ТОЛЬКО «Город»/«Страна»:
    «Азербайджан (перевод)» - это метка КП, а не реальный город на странице, и её
    сверка давала ложное ✗ «города нет на странице». ЧИСТАЯ функция (юнит-тест)."""
    if "(перевод)" not in (city or "").lower():
        return fields
    for f in fields:
        if f.get("field") in ("Город", "Страна"):
            f.update(status="na", found="–",
                     note="переводная версия сайта - город не сверяем (это метка КП)")
    return fields
...
def _merge_подмена(fld, r, from_kp, target, kind, метка, dial='7') -> None:
    ...
    # Расхождение (✗) пишем ЕДИНООБРАЗНО, как везде: «телефон на сайте не
    # совпадает с КП». Что в КП и что на сайте - видно в колонках «КП»/«Сайт».
    if from_kp:
        if st == 'replaced_ok':
            fld['status'] = 'ok'
            ...
            # Подмена не сработала: на сайте остался общий номер, а не из КП.
            fld['status'] = 'bug'
            fld['found'] = shown
            fld['note'] = "телефон на сайте не совпадает с КП"
        # no_element/error - оставляем статический вердикт по коду.
    else:  # рекламного номера в КП НЕТ (сверять не с чем) - не ошибка, прочерк «–»
        # Городам без своего рекламного номера в коде подставляется общий/
        # глобальный подменный - это не расхождение КП. Показываем «–» с
        # пояснением (по просьбе заказчика: «рекламного номера нет в кп, на
        # сайте общий»), а не ✗.
        fld['status'] = 'na'
        fld['found'] = shown if shown != "–" else fld.get('found', '–')
        fld['note'] = "рекламного номера в КП нет, на сайте общий"
...
def _check_site(a, domains, proxy) -> list:
    ...
    for dom, row in domains:
        ...
        # Адрес не нашли на главной (found «–») - догрузим «Контакты» и пересверим.
        # Триггер по «не найдено» (а не по статусу): теперь «не найден» = ✗ (bug),
        # но «Контакты» всё равно надо проверить - там адрес может быть.
        if (html and not err and _af
                and str(_af.get("found") or "").strip() in ("", "–")):
        ...
        # Переводная копия сайта: сверяем контакты как обычно, гасим только «Город».
        var["fields"] = _только_почта_для_перевода(row.city, var["fields"])
        var["error"] = ""
        результаты.append(var)
...
_ЛЕГЕНДА = [
    ...
    ("", False),
    ("✓  – и в КП, и на сайте значение ЕСТЬ и они совпадают "
