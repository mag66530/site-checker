...
_ЛЕГЕНДА = [
    ...
    ("", False),
    ("Телефоны «поиск» / «реклама» / «общий» проверяются по правилу: номер на сайте "
     "входит в набор номеров города из КП (на статике виден один номер).", False),
    ("", False),
    ("Порядок строк — как в «Карте присутствия»: сначала Россия, затем остальные "
     "страны по списку КП.", False),
]
