...
TECH_PAGE_PATHS = {
    ...
    # МПИ - БЕЗ завершающего слеша, в отличие от остальных проектов: сайт на
    # Next.js, и адреса везде slash-less (как и в каталоге - /catalog/<slug>).
    # Со слешем страницы уйдут в редирект/404 и осядут в отчёте ложной ошибкой.
    # Список полный: в шапке и подвале сайта других служебных страниц нет
    # (политики конфиденциальности/согласия на обработку ПД у проекта пока не
    # существует - в отличие от СМУ/ИМП).
    'mpi': [
