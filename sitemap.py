...
async def _fetch_one(session, url: str, timeout_s: int = 30) -> str:
    ...
    *,
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        while queue and processed < max_sitemaps:
            ...
            try:
                xml = await _fetch_one(session, next_url)
...
async def load_product_pathnames(
    ...
    *,
    ...
    try:
        all_urls = await collect_all_urls(sitemap_url, user_agent=user_agent, log=log)
    except Exception as e:
        return {
            'pathnames': [], 'fetched_at': int(time.time() * 1000),
