...
Область доступа - drive (весь Диск аккаунта).

Раньше просили drive.file («только свои файлы»), и это ломало главное: папку,
созданную ЧЕЛОВЕКОМ вручную, приложение не видит вовсе - files.get отвечает
404, а поиск по имени возвращает пусто. Писать в такую папку по её ID можно, а
найти в ней «2026» - нельзя, поэтому чекер каждый раз заводил СВОЮ «2026»
рядом с существующей. Никаким запросом это не обходится: нет права читать -
нет и ответа. Чтобы складывать отчёты в готовую структуру проекта, нужно право
эту структуру видеть.
...
# Весь Диск, а не drive.file: иначе не видно папок, созданных вручную, и вместо
# них плодятся дубли (см. докстринг модуля).
SCOPES = [DRIVE_SCOPE, "https://www.googleapis.com/auth/userinfo.email"]


def полный_доступ_к_диску(выдано: str) -> bool:
    """Выдан ли scope на ВЕСЬ Диск. Сравниваем по списку целиком, а не подстрокой:
    в «…/auth/drive.file» подстрока «…/auth/drive» тоже есть, и проверка «drive
    in scope» пропускала узкий доступ как годный.
    ЧИСТАЯ функция - проверяется юнит-тестом."""
    return DRIVE_SCOPE in str(выдано or "").split()
...
def exchange_code(client_id: str, client_secret: str, code: str,
    ...
    if not полный_доступ_к_диску(выдано):
        raise RuntimeError(
            "доступ к Google Диску не выдан полностью. В окне Google отметьте "
            "флажок про просмотр, создание и изменение файлов на Диске (он "
            "снимается вручную) и повторите подключение. Без права ВИДЕТЬ Диск "
            "чекер не найдёт уже созданные вами папки и заведёт рядом свои.")
