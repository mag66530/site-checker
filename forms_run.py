...
def main() -> int:
    ...
    _stamp('СБОРКА проверки форм: mpk-fix-20 · «Обработка ошибок»: перед пробой '
           'возвращаем в форму валидные значения - иначе на форме с собственной '
           'валидацией («Обратная связь») клик по пустой форме не давал запроса и '
           'строка уходила в прочерк; комментарии на одинаковые ошибки теперь '
           'одинаковые и короткие (согласие текстом, двойная отправка)')
