...
PROJECT_NAMES = {
    ...
    'mtt_am': 'МТТ - Меттранстерминал (Армения)',
}
...
def _память(пометка: str = '') -> None:
    """Расход памяти контейнера в лог. Пишем ПО ХОДУ прогона: когда контейнер
    убивают за превышение, лог умирает вместе с ним, и посмертно смотреть
    нечего - зато страница тайлит его вживую. Локально строки нет (cgroup
    только в контейнере)."""
    try:
        import mem_usage
        строка = mem_usage.строка()
    except Exception:  # noqa: BLE001
        строка = ''
    if строка:
        _stamp(f'{строка}{f" — {пометка}" if пометка else ""}')
...
def main() -> int:
    ...
    ap.add_argument('--workers', type=int, default=None,
                    help='Сколько форм проверять одновременно (1..5). У каждого '
                         'потока свой Chromium, поэтому это же число браузеров '
                         'в памяти: в облаке они делят контейнер с приложением. '
                         'По умолчанию - значение из движка (2).')
    a = ap.parse_args()
    ...
    # Серверную валидацию (отправка невалидных данных в обход клиентской проверки)
    # включаем флагом --server-validation-probe ИЛИ в конфиге проекта
    # (ПРОБА_СЕРВЕРНОЙ_ВАЛИДАЦИИ = True) - тогда идёт по умолчанию. Нужно проектам,
    # где критично ловить приём заявки без обязательных полей (напр. МПК: сайт
    # принимает форму без телефона). Работает и локально, и на сайте (тот же
    # forms_run под капотом). Формы заказа проба всё равно пропускает.
    import re as _re_srv
    _cfg_txt = src_config.read_text(encoding='utf-8')
    серверная_валидация_вкл = a.server_validation_probe or bool(_re_srv.search(
        r'ПРОБА_СЕРВЕРНОЙ_ВАЛИДАЦИИ\s*=\s*True', _cfg_txt))
    # Активная проба лимита/спама (до 3 быстрых повторных отправок): тестирует и
    # двойную отправку, и спам-лимит одним залпом. Включаем флагом
    # --rate-limit-probe ИЛИ конфигом (ПРОБА_ЛИМИТА = True). Нужна там, где кнопку
    # перекрывает оверлей: тогда обычную двойную отправку не проверить (тул её
    # пропускал → «под вопросом»), а залп жмёт JS-кликом сквозь оверлей и даёт
    # честный вердикт. Создаёт тест-заявки - формы заказа проба пропускает.
    лимит_вкл = a.rate_limit_probe or bool(_re_srv.search(
        r'ПРОБА_ЛИМИТА\s*=\s*True', _cfg_txt))
    ...
    # Версия-штамп: чтобы В ЛОГЕ было видно, какая сборка реально запущена (иначе
    # не отличить «правка не сработала» от «запущен старый код из другой папки»).
    _stamp('СБОРКА проверки форм: mpk-fix-24 · зависание «Обратной связи» убрано в '
           'корне: переход страницы больше НЕ роняем через сеть (это и ставило '
           'браузер на ERR_FAILED), а отменяем заранее перехватчиком submit - '
           'страница остаётся на месте. Форма без запроса, но с сабмитом = '
           'навигационная → «Обработка ошибок: корректно»')
    _stamp(f'ПРОВЕРКА ФОРМ СТАРТ - проект {name}'
           + (f' - городов: {len(run_cities)}' if run_cities and run_cities[0][0] else ''))
    try:
        sys.path.insert(0, str(ENGINE))
        from test_all import параллельных_форм as _потоки
        _stamp(f'Одновременно проверяем форм: {_потоки(a.workers)} '
               f'(столько же браузеров в памяти).')
    except Exception:  # noqa: BLE001
        pass
    if a.xss_probe:
    ...
    if лимит_вкл:
        _stamp('🚦 Активная проверка лимита запросов ВКЛючена: до 3 быстрых '
               'повторных отправок на КАЖДУЮ форму, ВКЛЮЧАЯ форму заказа. Если '
               'лимита нет - в админке появятся до 3 тест-заявок (а на чекауте '
               'возможен повторный заказ) - удалите их после прогона.')
    ...
    try:
        ...
        for i, (city, city_url, city_mail) in enumerate(run_cities):
            ...
            _память(f'перед городом {city or "основной сайт"}')
            ...
            run_test(
                ...
                валидация_проба=серверная_валидация_вкл,
                лимит_проба=лимит_вкл,
                проверять_цели=a.check_goals,
                потоков=a.workers,
            )

            _память(f'после города {city or "основной сайт"}')
        ...
        if not os.environ.get('FORMS_NO_CONSOLIDATE'):
            try:
                ...
                _сведено = консолидировать_форм_строки('log_forms.xlsx') is not False
            except Exception as e:  # noqa: BLE001
                _сведено = False
                _stamp(f'⚠️ Консолидация отчёта не выполнена: {e}')
        if not _сведено:
            _stamp('⚠️ ВНИМАНИЕ: отчёт не сведён (чаще всего log_forms.xlsx открыт '
                   'в Excel). В «Логах» останутся строки каждой проверки, а не по '
                   'одной на форму. Закройте файл и прогоните проверку заново.')

        # Колонку «Комментарий» листа «Логи» тянем шире - ДО матрицы. ВАЖНО: это
        # load+save, а openpyxl при перечитывании сбрасывает размер окон заметок
        # на дефолт (144x79) и заново пишет SizeWithCells. Поэтому подгонку делаем
        # ПЕРЕД матрицей, чтобы матрица (с крупными заметками) сохранялась ПОСЛЕДНЕЙ
        # и её заметки не затирались. Ширина колонок при перечитывании сохраняется.
        try:
        ...
        # ПОСЛЕ консолидации (нужны уже сведённые строки) и ПОСЛЕДНЕЙ - чтобы её
        # заметки (крупные, без SizeWithCells) не сбивались повторным сохранением.
        # Отключается переменной FORMS_NO_MATRIX=1.
    ...
    if rc == 0:
        ...
        if not a.check_goals:
            try:
                ...
                # Диск: <Год>/Сайт чекер/<Месяц>/<Дата>/Проверка форм/<файл>.
                if _отчёт.is_file():
                    try:
