...
def main() -> int:
    ...
    _stamp('СБОРКА проверки форм: mpk-fix-23 · «Обратная связь» отправляется '
           'ПЕРЕХОДОМ страницы (action /contacts/index/post/): оборванный переход '
           'ронял браузер на ERR_FAILED и прогон вис. Теперь такой переход '
           'помечается отдельно, страница формы возвращается назад и заполняется '
           'заново, а вердикт - «корректно» (о сбое сообщает браузер)')
