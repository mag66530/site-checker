...
def main() -> int:
    ...
    _stamp('СБОРКА проверки форм: mpk-fix-17 · «Ошибки валидации» на модалках '
           'звонка: реальная пустая отправка при ВИДИМОЙ форме ловит серверную '
           'подсказку «обязательно» (bookacall); «Обработка ошибок»: сначала ищем '
           'показанную ошибку (модалка показывает «Попробуйте позже» → корректно, '
           'не «ложный успех»); телефон с маской +7 дозаполняется местным форматом '
           '(карточка теперь отправляется, Enter работает); «Ссылка на политику» '
           'учитывает ссылку РЯДОМ с формой; «Спам активно» судится по факту '
           'ушедших POST-ов (3 отправки), а не по визуальному подтверждению')
