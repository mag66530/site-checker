...
def main() -> int:
    ...
    серверная_валидация_вкл = a.server_validation_probe or bool(_re_srv.search(
        r'ПРОБА_СЕРВЕРНОЙ_ВАЛИДАЦИИ\s*=\s*True',
        src_config.read_text(encoding='utf-8')))
    ...
    _stamp('СБОРКА проверки форм: mpk-fix-8 · попап-кука mycity + серверная '
           'валидация из конфига + checkout button.btn-checkout + дозаполнение полей')
