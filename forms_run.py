...
def main() -> int:
    ...
    _stamp('СБОРКА проверки форм: mpk-fix-19 · «Обработка ошибок» проверяется и на '
           'обычных (не AJAX) формах - прочерка больше нет, между пробами добавлены '
           'паузы, чтобы чужая поздняя панель не портила вердикт; «Подсказки полей» '
           'ловят случай, когда подсказки есть НЕ У ВСЕХ полей (было ложное ✓); '
           '«Серверная валидация» заполняется и на форме заказа - по полям, без '
           'тест-отправок (буквы в телефоне ловятся везде одинаково)')
