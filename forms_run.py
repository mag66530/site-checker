...
def main() -> int:
    ...
    _stamp('СБОРКА проверки форм: mpk-fix-16 · буквы в телефоне → «Серверная '
           'валидация: УЯЗВИМА»; снимок/восстановление формы вокруг проб (модалка '
           'больше не прячется от оборванного POST); ссылка на обработку ПД рядом '
           'с формой; короткие комментарии вместо «не удалось определить»')
