...
def main() -> int:
    ...
    _stamp('СБОРКА проверки форм: mpk-fix-22 · НАЙДЕН корень «ложного успеха»: '
           'проба обработки ошибок гасила только ПЕРВЫЙ POST, а повторный клик '
           'уходил на сервер по-настоящему - модалка показывала «Спасибо» от '
           'СВОЕЙ ЖЕ заявки. Теперь гасим ВСЕ отправки и не кликаем повторно, если '
           'запрос уже ушёл; молчание формы на сбой - вердикт «молчит» (⚠), а не '
           'прочерк')
