...
def main() -> int:
    ...
    ap.add_argument('--requests', action='store_true',
                    help='Быстрый режим «по коду»: обычные формы проверяются '
                         'запросом (HTML+POST через requests), без запуска '
                         'браузера - заявка отправляется, но браузерные проверки '
                         '(XSS/двойная отправка/серверная валидация/лимит/стили) '
                         'не выполняются. Многошаговые сценарии заказа всё равно '
                         'идут через браузер.')
    a = ap.parse_args()
    ...
    # Режим «по коду» (requests): обычные формы проверяются запросом без браузера
    # (быстро). Движок читает ФОРМЫ_ЧЕРЕЗ_REQUESTS из config - дописываем флаг.
    if a.requests:
        base_config = base_config.rstrip() + '\n\nФОРМЫ_ЧЕРЕЗ_REQUESTS = True\n'
        _stamp('⚡ Режим «по коду» (requests): обычные формы - запросом, без '
               'браузера. Браузерные пробы (XSS/двойная/валидация/лимит) не идут; '
               'многошаговые сценарии заказа всё равно через браузер.')
