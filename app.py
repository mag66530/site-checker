...
# Google возвращает человека после согласия на БАЗОВЫЙ адрес приложения
# (?code=…&state=gdrive:<проект>), а не на страницу настроек - поэтому ответ
# принимаем здесь, до навигации. Если параметров нет, вызов ничего не делает.
auth.handle_gdrive_oauth_redirect()
...
# slug (URL-путь) → страница: строим сами. url_path у st.Page вычисляется лениво
# только внутри st.navigation, до неё недоступен, поэтому не полагаемся на него.
_slug_page = {}
...
for _key in auth.APP_TAB_KEYS:
    if _key in _allowed and _key in _PAGE_DEFS:
        ...
        _pg = st.Page(_path, title=_title, icon=_icon, default=not pages)
        pages.append(_pg)
        _slug_page[_os.path.splitext(_os.path.basename(_path))[0]] = _pg
...
if _user and _user.get('role') in ('manager', 'admin'):
    _pg = st.Page(auth.manager_cabinet_page, title='Кабинет руководителя',
                  icon='🗂', url_path='cabinet')
    _mgmt_pages.append(_pg)
    _slug_page['cabinet'] = _pg
...
if _user and auth.live_settings_projects(_user):
    _pg = st.Page(auth.project_settings_page, title='Настройки проекта',
                  icon='🔑', url_path='project-settings')
    _mgmt_pages.append(_pg)
    _slug_page['project-settings'] = _pg
if _user and _user.get('role') == 'admin':
    _pg = st.Page(auth.admin_panel_page, title='Админ-панель', icon='⚙️',
                  url_path='admin')
    _mgmt_pages.append(_pg)
    _slug_page['admin'] = _pg

# Вернуться на страницу, где пользователь обновил вкладку (F5). При обновлении
# cookie-probe в require_login делает st.stop() без навигации → Streamlit
# сбрасывает URL на корень; take_return_slug() отдаёт запомненный до probe slug
# ОДИН раз, и мы переключаемся на ту страницу.
_want_slug = auth.take_return_slug()
if _want_slug and _want_slug in _slug_page:
    try:
        st.switch_page(_slug_page[_want_slug])
    except Exception:
        pass
