...
STATUS_ERROR = 'Ошибка'
# Обрабатываем СТРОГО эти два статуса (каждый своим путём). «Начато»/«Идёт
# проверка» НЕ трогаем: у них нет «Проверить исправление», а путь через
# «Подробности → Начать новую проверку» ПЕРЕЗАПУСКАЕТ уже идущую проверку.
# «?» (не распознали) тоже не трогаем - лучше пропустить, чем перезапустить;
# сырые строки «?» пишутся в лог, по ним пополняем паттерны ниже.
STATUS_PROCESS = (STATUS_NOT_STARTED, STATUS_ERROR)
# Паттерны статусов: ищем БЕЗ регистра, ё=е; порядок важен - «не начато»
# должен матчиться раньше «начато» (вторая - подстрока первой).
_STATUS_PATTERNS = (
    ('не начато', STATUS_NOT_STARTED),
    ('not started', STATUS_NOT_STARTED),
    ('идет проверка', 'Идёт проверка'),
    ('проверка начата', 'Начата'),
    ('начата', 'Начата'),
    ('начато', 'Начата'),
    ('ошибка', STATUS_ERROR),
...
def _status_from_text(txt: str) -> str:
    ...
    поэтому ищем подстрокой без регистра (ё=е), а не точным равенством.
    Источник не найден → «?»: искать статус по ВСЕЙ строке нельзя -
    «Ошибка» из названия причины дала бы ложный статус."""
    tail = None
    for src in ('Системы Google', 'Сайт', 'Google systems', 'Website'):
        if src in txt:
            tail = txt.split(src, 1)[1]
            break
    if tail is None:
        return '?'
    tail = tail.lower().replace('ё', 'е')
...
async def _details_newcheck(page, res: dict, reason: dict, dry_run: bool) -> dict:
    """Со СТРАНИЦЫ ПРИЧИНЫ (уже открыта): «Подробности» → «Начать новую
    проверку» → подтверждение диалога. Возвращает заполненный res и уводит
    страницу назад к таблице причин."""
    ...
    try:
        ...
        res['message'] = 'ни «Проверить исправление», ни «Подробности» нет'
        _log(f'  {reason["name"][:50]} - кнопок нет (проверка недоступна)', 'warn')
        await page.go_back()
    ...
    if dry_run:
        res['status'] = 'dry_run'
        res['message'] = '«Подробности» найдена, клик пропущен'
        _log(f'  [DRY RUN] {reason["name"][:50]} - путь есть', 'ok')
        await page.go_back()
        await page.wait_for_timeout(1500)
        return res
    ...
    try:
        ...
        # span сам может не быть click-таргетом - JS-фоллбэк
        try:
        ...
        # (кнопка «Начать новую проверку» лишь открывает окно).
        _confirmed = False
        ...
        if _started or _confirmed:
            res['status'] = 'ok'
            res['message'] = 'новая проверка запущена (через «Подробности»)'
            _log(f'  ✓ новая проверка: {reason["name"][:50]}', 'ok')
        else:
            res['status'] = 'ok'
            res['message'] = 'клик сделан (подтверждение не распознано)'
            _log(f'  ✓ клик сделан (без подтверждения): {reason["name"][:50]}', 'ok')
    except Exception:
        res['status'] = 'warn'
        res['message'] = 'кнопки «Начать новую проверку» нет'
        _log('  нет «Начать новую проверку»', 'warn')

    # Назад к таблице. Возможный открытый диалог закрываем Esc - иначе
    # go_back не уводит и следующий фильтр не переключится.
    try:
...
async def _validate_error(page, rid: str, reason: dict, dry_run: bool) -> dict:
    ...
    try:
        ...
        return await _details_newcheck(page, res, reason, dry_run)
    except Exception as e:
        res['status'] = 'error'
        res['message'] = str(e)
...
async def _validate_one(page, rid: str, reason: dict, dry_run: bool) -> dict:
    ...
    try:
        ...
        try:
            ...
            # Кнопки нет - у этой причины путь через «Подробности → Начать
            # новую проверку» (как у статуса «Ошибка»). Мы уже на странице
            # причины - пробуем его, а не пропускаем.
            return await _details_newcheck(page, res, reason, dry_run)
...
async def process_resource(page, rid: str, dry_run: bool,
    ...
    # Диагностика нераспознанных статусов: сырой текст первых строк - чтобы
    # по логу видеть, как GSC поменял вёрстку, и добавить статус в парсер.
    _unk = [r for r in reasons if r['status'] == '?'][:2]
    for r in _unk:
        _log(f'    [?] сырая строка: {r["text"][:120]}')
