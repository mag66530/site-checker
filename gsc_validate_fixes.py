...
STATUS_ERROR = 'Ошибка'
# Обрабатываем эти статусы (каждый своим путём). «?» - статус не распознался
# (GSC периодически меняет вёрстку строк): такие причины НЕ пропускаем, а
# идём универсальным путём - есть «Проверить исправление» → кликаем, нет →
# «Подробности → Начать новую проверку». Уже идущую/пройденную проверку
# Google перезапустить не даст (кнопок не будет) - лишнего не натворим.
