...
def _norm_addr(s: Optional[str]) -> str:
    ...
    # Схлопываем удвоенные БУКВЫ (не цифры!): «Новороссийская» ≈ «Новоросийская»
    # (частая опечатка в КП - одна «с»). Номера домов (236, 1) не трогаем.
    s = re.sub(r'([a-zа-яё])\1+', r'\1', s)
...
def extract_site_contacts(html: str) -> dict:
    ...
    if not addr:
        m = re.search(r'.{0,40}(?:улиц|\bул\.?\s?[А-ЯЁ]|проспект|пр-?кт|\bпр\.\s|'
                      r'шоссе|переул|\bпер\.|набережн|\bнаб\.|бульвар|\bб-р|'
                      r'микрорайон|\bмкр)\.?.{0,40}', text, re.IGNORECASE)
        if m:
...
def check_variables(html: str, domain: str) -> dict:
    ...
    if not row.address:
        ...
        # site['address'] берётся из шапки/подвала; на «Контактах» адрес в основном
        # блоке - поэтому дополнительно ищем по ВСЕЙ странице (метка «Адрес:»).
        site_addr = (site.get("address") or "").strip() or _site_address_full(html)
        if site_addr:
