...
def _site_address_full(html: str) -> str:
    ...
    # - это не адрес, лучше показать «не распознан», чем мусор.
    if not re.search(r'\d', cap):
        return ''
    if not re.search(r'\b(?:ул|улиц|просп|проспект|пр-?кт|шоссе|переул|пер|наб|'
                     r'набережн|бульвар|б-р|мкр|микрорайон|проезд|тракт|'
                     r'площад|пл)\b', cap, re.I):
        return ''
    return cap
