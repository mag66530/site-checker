...
def normalize_phone(s: Optional[str]) -> str:
    ...
    Узбекистан +998, Киргизия +996, Азербайджан +994 → 9 цифр; Армения +374 →
    8 цифр. Excel иногда хранит номер числом («…448.0») - отбрасываем «.0».

    Армению добавили 19.08.2026 по живой находке на МТТ: номер Еревана
    «37 (443) 154598» (те же цифры, что и на сайте) без кода 374 обрезался до
    последних десяти цифр и показывался как российский «+7 (744) 315-45-98» -
    сверка «Общий Город» давала ложное расхождение на ровном месте.
    """
    if s is None:
        return ''
    s = str(s)
    ...
    if not d:
        return ''
    # Армения +374: нац. номер 8 цифр (код страны + 8 = 11).
...
_PHONE_FIND = re.compile(
    ...
    r'|(?<!\d)\+?99[64](?:[\s\-()]*\d){9}(?!\d)'
    # Армения +374: код страны + 8 цифр при любой группировке. На met-trans.am
    # номер записан как «37 (443) 154598» - скобка стоит ВНУТРИ кода страны,
    # поэтому разделители разрешены и между его цифрами.
    r'|(?<!\d)\+?3[\s\-()]*7[\s\-()]*4(?:[\s\-()]*\d){8}(?!\d)'
...
def split_phones(s: Optional[str]) -> list[str]:
    ...
    for m in _PHONE_FIND.findall(str(s)):
        ...
        if len(n) == 10 and n.startswith('0'):
            continue
        # 8 - длина армянского нац. номера (+374 43 154598). Раньше нижней
        # границей было 9, и найденный армянский номер отбрасывался уже ПОСЛЕ
        # нормализации - в сверку он не попадал вовсе.
        if 8 <= len(n) <= 10 and n not in out:
            out.append(n)
    return out
...
def phones_in_cell(s: Optional[str]) -> list[str]:
    ...
    разделителям и нормализуем каждый кусок; берём только валидные номера.

    Нижняя граница - 8 цифр: столько в национальном номере Армении
    (+374 43 154598). При прежних «9-10» армянский номер Еревана из ячейки не
    доставался вовсе: сверка получала пустой список, показывала сырую строку
    «37 (443) 154598» и писала расхождение, хотя на сайте номер тот же."""
    if not s:
        return []
    out = []
...
_DIAL_BY_COUNTRY = {
    ...
    # Армения: нац. номер 8 цифр (+374 43 154598).
    'армения': '374',
}
_DIAL_BY_TLD = {'by': '375', 'kg': '996', 'uz': '998', 'az': '994', 'am': '374'}
...
def _fmt(nat: str, dial: str = '7') -> str:
    ...
    if dial == '374' and len(nat) == 8:              # +374 (43) 15-45-98
        return f'+374 ({nat[:2]}) {nat[2:4]}-{nat[4:6]}-{nat[6:]}'
